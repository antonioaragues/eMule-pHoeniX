...
CString ReverseDnsLookup(DWORD dwIP)
{
	...
	if (thePrefs.GetWebCacheLastGlobalIP() == dwIP
		&& thePrefs.GetLastResolvedName() != _T("")
		...
		if (pfnDnsQueryConfig && pfnDnsQuery && pfnDnsRecordListFree)
		{
			...
			if (nDnsState == 0)
			{
				if (AtlIsValidAddress(pDnsRecords, sizeof(*pDnsRecords) - sizeof(pDnsRecords->Data) + sizeof(pDnsRecords->Data.PTR), FALSE))
				{
					...
					if (pDnsRecords) // [TPT] - Pawcio fix
						(*pfnDnsRecordListFree)(pDnsRecords, DnsFreeRecordListDeep);
				}
			}
			else{
				if (thePrefs.GetVerbose())
					theApp.QueueDebugLogLine(false, _T("PeerCache ReverseDNS: Failed to resolve address \"%s\" - %s"), strDnsQuery, GetErrorMessage(nDnsState, 1));
			}
	...
	// [TPT]
		...
		AddLogLine(false, _T("dnsapi.dll not found in your system\nIt's necessary to properly work PeerCache"));

	// [TPT] - WebCache	
	// Superlexx - no need to rape root DNS servers
	thePrefs.SetWebCacheLastGlobalIP(dwIP);
	thePrefs.SetLastResolvedName(strHostName);
	thePrefs.SetWebCacheLastSearch(GetTickCount()); // [TPT]
	// [TPT] - WebCache	
