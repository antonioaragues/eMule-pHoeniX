...
#include "DownloadClientsCtrl.h"// [TPT] - MFCK [addon] - New Tooltips [Rayita]
...
CMuleListCtrl::CMuleListCtrl(PFNLVCOMPARE pfnCompare, DWORD dwParamSort) {
	...
	m_dwParamSort.AddHead(dwParamSort);	// [TPT] - SLUGFILLER: multiSort
...
void CMuleListCtrl::SetColors(LPCTSTR pszLvKey) {
	...
	m_crPhoenix		= MLC_RGBBLEND(255, m_crWindow, 8); // [TPT]
}
...
int CMuleListCtrl::UpdateLocation(int iItem) {
	...
	if(notFirst) {
		...
		int iResult = MultiSortProc(dwpItemData, GetParamAt(pos, iNewIndex));	// [TPT] - SLUGFILLER: multiSort
		if(iResult < 0) {
			...
			while(iDist > 1) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(posPrev, iNewIndex - iDist)) < 0) {	// [TPT] - SLUGFILLER: multiSort
			...
			while(--iNewIndex >= 0) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(pos, iNewIndex)) >= 0)	// [TPT] - SLUGFILLER: multiSort
					break;
			}
	...
	if(notLast) {
		...
		if(iResult > 0) {
			...
			while(iDist > 1) {
				...
				if(MultiSortProc(dwpItemData, GetParamAt(posNext, iNewIndex + iDist)) > 0) {	// [TPT] - SLUGFILLER: multiSort
					iNewIndex = iNewIndex + iDist;
					pos = posNext;
				} else {
					posNext = pos;
	...
	case WM_NOTIFY:
		if(wParam == 0) {
			if(((NMHDR*)lParam)->code == NM_RCLICK) {
				...
				tmColumnMenu->SetSelectedBarColor(RGB(242,120,114));
				...
				for(int iCurrent = 1; iCurrent < iCount; iCurrent++) {
					...
					tmColumnMenu->AppendODMenu(MF_STRING | m_aColumns[iCurrent].bHidden ? 0 : MF_CHECKED, new CMenuXPText(MLC_IDC_MENU + iCurrent,item.pszText));
				}
	...
	case LVM_INSERTITEMW:
		//try to fix position of inserted items
		{
			...
			if(notFirst) {
				int iNewIndex = iItem - 1;
				POSITION pos = m_Params.FindIndex(iNewIndex);
				int iResult = MultiSortProc(pItem->lParam, GetParamAt(pos, iNewIndex));	// [TPT] - SLUGFILLER: multiSort
				if(iResult < 0) {
					POSITION posPrev = pos;
					int iDist = iNewIndex / 2;
					while(iDist > 1) {
						...
						if(MultiSortProc(pItem->lParam, GetParamAt(posPrev, iNewIndex - iDist)) < 0) {	// [TPT] - SLUGFILLER: multiSort
							iNewIndex = iNewIndex - iDist;
							pos = posPrev;
						} else {
							posPrev = pos;
					...
					while(--iNewIndex >= 0) {
						m_Params.GetPrev(pos);
						if(MultiSortProc(pItem->lParam, GetParamAt(pos, iNewIndex)) >= 0)	// [TPT] - SLUGFILLER: multiSort
							break;
					}
			...
			if(notLast) {
				...
				if(iResult > 0) {
					...
					while(iDist > 1) {
						...
						if(MultiSortProc(pItem->lParam, GetParamAt(posNext, iNewIndex + iDist)) > 0) {	// [TPT] - SLUGFILLER: multiSort
							iNewIndex = iNewIndex + iDist;
							pos = posNext;
						} else {
							posNext = pos;
