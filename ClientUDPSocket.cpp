...
#include "WebCachedBlockList.h" // [TPT] - Webcache 1.9 beta3 Superlexx - managed OHCB list
// [TPT] - WebCache
...
void CClientUDPSocket::OnReceive(int nErrorCode)
{
	...
	// [TPT] - end
	if (length >= 1 && !(theApp.ipfilter->IsFiltered(sockAddr.sin_addr.S_un.S_addr) || theApp.clientlist->IsSnafuClient(sockAddr.sin_addr.S_un.S_addr))) // [TPT] - eWombat [SNAFU]
    {
		CString strError;
		try
		{
		...
		case OP_REASKCALLBACKUDP:
		{
			...
			if( buddy )
			{
				if( size < 17 || buddy->socket == NULL )
					break;
				uchar check[16];
				MEMCOPY(&check, packet, 16);
				if( !memcmp(&check, buddy->GetBuddyID(), 16) )
				{
					MEMCOPY(packet+10, &ip, 4);
					MEMCOPY(packet+14, &port, 2);
			...
			if (sender)
			{
				...
				if (md4cmp(reqfilehash, sender->GetUploadFileID()) == 0)
				{
					...
					else if (sender->GetUDPVersion() > 2)
					{
						...
						{
							bool isPf = reqfile->IsPartFile();
							if(isPf) // [TPT] - enkeyDev: ICS
...
int CClientUDPSocket::SendTo(char* lpBuf,int nBufLen,uint32 dwIP, uint16 nPort){
	...
	// [TPT] - Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	if(result > 0){
		theApp.pBandWidthControl->AddeMuleOutUDPOverall(result);
	}
	// Maella end
