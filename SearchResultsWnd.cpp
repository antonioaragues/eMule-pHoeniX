...
#include "TransferWnd.h" // [TPT] - khaos::categorymod
...
void CSearchResultsWnd::OnInitialUpdate()
{
	...
	//AddAnchor(IDC_STATIC_DLTOof,BOTTOM_LEFT); // [TPT] - khaos::categorymod
	//AddAnchor(IDC_CATTAB2,BOTTOM_LEFT,BOTTOM_RIGHT); // [TPT] - khaos::categorymod
...
void CSearchResultsWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_CATTAB2, m_cattabs); // [TPT] - khaos::categorymod
}
...
CString	CSearchResultsWnd::CreateWebQuery(SSearchParams* pParams)
{
	...
	// [TPT] - [MoNKi: -eMugle.com as search method-]
	case SearchTypeEmugle:
		query = _T("http://www.emugle.com/search.php?");
		query += _T("q=") + ToQueryString(pParams->strExpression);
		if (pParams->strFileType == ED2KFTSTR_AUDIO)
			query += _T("&t=Audio");
		else if (pParams->strFileType == ED2KFTSTR_VIDEO)
			query += _T("&t=Video");
		else if (pParams->strFileType == ED2KFTSTR_ARCHIVE)
			query += _T("&t=Archive");
		else if (pParams->strFileType == ED2KFTSTR_IMAGE)
			query += _T("&t=Image");
		else if (pParams->strFileType == ED2KFTSTR_CDIMAGE)
			query += _T("&t=CD%20Image");

		if (pParams->ulMinSize > 0)
			query.AppendFormat(_T("&mins=%u"),pParams->ulMinSize / 1048576);
		
		if (pParams->ulMaxSize > 0)
			query.AppendFormat(_T("&maxs=%u"),pParams->ulMaxSize / 1048576);

		if (pParams->uAvailability > 0)
			query.AppendFormat(_T("&avai=%u"),pParams->uAvailability);

		query += _T("&sourceid=pHoeniX-mod");
...
void CSearchResultsWnd::DownloadSelected(bool bPaused)
{
	...
	// [TPT] - khaos::categorymod+ Category selection stuff...
	...
	while (pos != NULL)
	{
		...
		if (iIndex >= 0)
		{
			...
			// [TPT] - khaos::categorymod+ m_cattabs is obsolete.
			...
			*/
			...
			if(theApp.knownfiles->CheckAlreadyDownloadedFileQuestion(cur_file->GetFileHash(),cur_file->GetFileName()))
			{				
...
void CSearchResultsWnd::OnSysColorChange()
{
	...
	SetAllIcons();
	//searchlistctrl.CreateMenues(); [TPT] - New Menues
}
...
void CSearchResultsWnd::Localize()
{
	...
	//UpdateCatTabs(); // [TPT] - khaos::categorymod+
...
// [TPT] - khaos::categorymod+ These things are obsolete, replaced by the select cat dialog.
...
CString	CSearchResultsWnd::ToQueryString(CString str){
	// [TPT] - [MoNKi: -Fixed UTF-8 strings on web searchs-]
	/*
