...
static int __cdecl CmpFakeByHash_Lenght(const void* p1, const void* p2)
{
	...
	if (diff)
		return diff;
	return CompareUnsigned(pFake1->Lenght, pFake2->Lenght);
}
...
int CFakecheck::LoadFromFile(){
	...
	if (readFile!=NULL) {
		...
		Log(LOG_SUCCESS, _T("%i Fake Check reference loaded"), m_fakelist.GetCount());
...
bool CFakecheck::IsFake(uchar* Hash2test, uint32 lenght){
	...
	//[TPT] - temporal fix, the length value is not catched properly
	//so I must send the structure instead the hash alone
	//why does it happen in pHoeniX and not in other mods?:'(
	temporal = new Fakes_Struct;
	md4cpy(temporal->Hash,Hash2test);
	temporal->Lenght = lenght;

	Fakes_Struct** ppFound = (Fakes_Struct**)bsearch(&temporal, m_fakelist.GetData(), m_fakelist.GetCount(), sizeof(m_fakelist[0]), CmpFakeByHash_Lenght);
	delete temporal;
	temporal = NULL;
	if (ppFound)
	{
