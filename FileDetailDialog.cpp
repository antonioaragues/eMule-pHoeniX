...
IMPLEMENT_DYNAMIC(CFileDetailDialog, CModelessPropertySheet)	// SLUGFILLER: modelessDialogs

BEGIN_MESSAGE_MAP(CFileDetailDialog, CModelessPropertySheet)	// SLUGFILLER: modelessDialogs
	ON_WM_DESTROY()
...
void CFileDetailDialogInfo::RefreshData()
{
	...
	uint32  uWebcachedownloaded = 0;//jp webcache //[TPT] - Webcache 1.9 beta3
	...
	for (int i = 0; i < m_paFiles->GetSize(); i++)
	{
		...
		uDataRate += file->GetDownloadFileDatarate(); // [TPT]
...
BOOL CFileDetailDialogName::OnInitDialog()
{
	...
	for (int i = thePrefs.GetColumnSortCount(CPreferences::tableFilenames); i > 0; ) {
		i--;
		m_sortindex = thePrefs.GetColumnSortItem(CPreferences::tableFilenames, i);
		m_sortorder = thePrefs.GetColumnSortAscending(CPreferences::tableFilenames, i);
...
void CFileDetailDialogName::OnNMRclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	//pMenu->AddSideBar(new CMenuXPSideBar(17, "pHoeniX 1.0"));
	//pMenu->SetSideBarStartColor(RGB(255,0,0));
	//pMenu->SetSideBarEndColor(RGB(255,128,0));
	pMenu->SetSelectedBarColor(RGB(242,120,114));
	pMenu->AppendODMenu(MF_STRING | flag, new CMenuXPText(MP_MESSAGE, GetResString(IDS_TAKEOVER)));	
	pMenu->AppendODMenu(MF_STRING | flag, new CMenuXPText(MP_COPYSELECTED, GetResString(IDS_COPY)));	
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_RESTORE, GetResString(IDS_SV_UPDATE)));		
...
void CFileDetailDialogInterface::OpenDetailDialog(const CSimpleArray<CPartFile*>* paFiles, UINT uInvokePage, CListCtrlItemWalk* pListCtrl)
{
	...
	OpenPropertySheet(&aInterfaces, paFiles, uInvokePage, pListCtrl);
}
...
CModelessPropertySheet* CFileDetailDialogInterface::CreatePropertySheet(va_list args)
{
	...
	return STATIC_DOWNCAST(CModelessPropertySheet, new CFileDetailDialog(paFiles, uInvokePage, pListCtrl));
}
// SLUGFILLER: modelessDialogs
