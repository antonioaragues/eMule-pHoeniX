...
#define FULLACCESS ADS_RIGHT_GENERIC_ALL
	...
	// we dont check for success here, for emule will also run if one dir fails for some reason
	...
	bSucceeded = bSucceeded && SetObjectPermission(thePrefs.GetLinkDir(), FULLACCESS);//[TPT] - Save ed2klinks
	bSucceeded = bSucceeded && SetObjectPermission(thePrefs.GetAutoHLDir(), FULLACCESS);//[TPT] - Sivka AutoHL
	bSucceeded = bSucceeded && SetObjectPermission(thePrefs.GetSaveSourcesDir(), FULLACCESS);//[TPT] - Save sources
