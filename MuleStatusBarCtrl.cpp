...
{
//	EnableToolTips();// [TPT] - MFCK [addon] - New Tooltips [Rayita]
}
...
void CMuleStatusBarCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	...
	switch (iPane)
	{
		...
		case SBarChatMsg:
			if(!thePrefs.IsDynUpEnabled())//[TPT]
...
HICON CMuleStatusBarCtrl::GetTipInfo(CString &info)
{
	...
	switch (GetPaneAtPosition(pt))
	{
		...
		case SB_SERVER:
		{
			...
			if(server && (server = theApp.serverlist->GetServerByAddress(server->GetAddress(), server->GetPort())))
...
#ifndef COMPACT
		case SB_MSG:
			if (!GetTipText(SB_MSG).IsEmpty())
			
				if(!thePrefs.IsDynUpEnabled())// [TPT] - FIX
				{
				if(theApp.emuledlg->message == true)
					info = GetResString(IDS_TT_SBMSG);
				else
					info = GetResString(IDS_TT_NOMSG);
				}
				else
					info = _T("Upload Speed Sense");
