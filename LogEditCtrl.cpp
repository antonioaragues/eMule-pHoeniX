...
void CLogEditCtrl::OnContextMenu(CWnd* pWnd, CPoint point){
	...
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_REMOVEALL, GetResString(IDS_PW_RESET)));	
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_SAVELOG, GetResString(IDS_SAVELOG) + _T("..."), theApp.LoadIcon(_T("diskStore"), 16, 16)));		
	pMenu->AppendSeparator();
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_AUTOSCROLL, GetResString(IDS_AUTOSCROLL)));	
	
	pMenu->EnableMenuItem(MP_COPYSELECTED, iSelEnd > iSelStart ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_REMOVEALL, iTextLen > 0 ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_SELECTALL, iTextLen > 0 ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_SAVELOG, iTextLen > 0 ? MF_ENABLED : MF_GRAYED);
	pMenu->CheckMenuItem(MP_AUTOSCROLL, m_bAutoScroll ? MF_CHECKED : MF_UNCHECKED);
...
// [TPT] - New Menu Styles BEGIN
void CLogEditCtrl::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
