...
BEGIN_MESSAGE_MAP(CHyperTextCtrl, CWnd)
	...
	// [TPT] - SLUGFILLER: hoverCaptureFix
...
LRESULT CHyperTextCtrl::OnPaint(WPARAM wParam, LPARAM lParam){
	...
	if((hcStruct.dwFlags & HCF_HIGHCONTRASTON)==HCF_HIGHCONTRASTON){
		bHighContrast=true;
		m_Font->GetLogFont(&hcLogFont);
		hcLogFont.lfUnderline = true;
...
void CHyperTextCtrl::OnMouseMove(UINT nFlags,CPoint pt){
	...
	if(PtInRect(rcClient, pt) && m_iLineHeight)
	{
			...
			for(CVisLine::iterator jt = it->begin(); jt != it->end(); jt++)
				if(pt.x >= jt->m_rcBounds.left && pt.x <= jt->m_rcBounds.right)
				{
					if(jt->m_pHyperLink != NULL)
					{
						HighlightLink(&*jt, pt);
						bFound = true;
						// [TPT] - SLUGFILLER: hoverCaptureFix - Tracking is better than capture
		...
		if(!bFound){
			RestoreLink();
			// [TPT] - SLUGFILLER: hoverCaptureFix remove - Removed capture code, tracking is better
		}
	}
	else
		RestoreLink();	// SLUGFILLER: hoverCaptureFix - Removed capture code, tracking is better
}
...
void CHyperTextCtrl::HighlightLink(CVisPart* Part, const CPoint& MouseCoords){
	...
	if((hcStruct.dwFlags & HCF_HIGHCONTRASTON)==HCF_HIGHCONTRASTON){
		bHighContrast=true;
		m_HoverFont.GetLogFont(&hcLogFont);
		hcLogFont.lfUnderline = true;
		hcFont.CreateFontIndirect(&hcLogFont);
		hOldFont = dc.SelectObject(&hcFont);
	}
	else{
		hOldFont = dc.SelectObject(&m_HoverFont);
	}
	// [TPT] - MoNKi: -Support for High Contrast Mode-
...
void CHyperTextCtrl::RestoreLink(){
	...
	if((hcStruct.dwFlags & HCF_HIGHCONTRASTON)==HCF_HIGHCONTRASTON){
		bHighContrast=true;
		m_LinksFont.GetLogFont(&hcLogFont);
		hcLogFont.lfUnderline = true;
		hcFont.CreateFontIndirect(&hcLogFont);
		hOldFont = dc.SelectObject(&hcFont);
	}
	else{
		hOldFont = dc.SelectObject(&m_LinksFont);
	}
	// [TPT] - MoNKi: -Support for High Contrast Mode-
	...
	// [TPT] - SLUGFILLER: hoverCaptureFix - No more tracking
	TRACKMOUSEEVENT csTME;
	csTME.cbSize = sizeof(csTME);
...
void CHyperTextCtrl::SetColors() {
	...
	if((hcStruct.dwFlags & HCF_HIGHCONTRASTON)==HCF_HIGHCONTRASTON){
		...
		m_LinkColor = GetSysColor( COLOR_BTNTEXT);
		m_HoverColor = GetSysColor( COLOR_BTNTEXT);
		return;
	}
	// [TPT] - MoNKi: -Support for High Contrast Mode-
