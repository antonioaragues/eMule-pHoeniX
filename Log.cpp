...
// [TPT] - Debug log
void AddPhoenixLogLine(bool bAddToStatusBar, LPCTSTR pszLine, ...)
{
	ASSERT(pszLine != NULL);

	va_list argptr;
	va_start(argptr, pszLine);
	AddPhoenixTextV(LOG_DEBUG | (bAddToStatusBar ? LOG_STATUSBAR : 0), DLP_DEFAULT, pszLine, argptr);
	va_end(argptr);	
}

void AddPhoenixLogLine(EDebugLogPriority Priority, bool bAddToStatusBar, LPCTSTR pszLine, ...)
{
	// loglevel needs to be merged with LOG_WARNING and LOG_ERROR later
	// (which only means 3 instead of 5 levels which you can select in the preferences)
	// makes no sense to implement two different priority indicators
	...
	AddPhoenixTextV(LOG_DEBUG | nFlag | (bAddToStatusBar ? LOG_STATUSBAR : 0), Priority, pszLine, argptr);
	va_end(argptr);	
}

void AddPhoenixTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	ASSERT(pszLine != NULL);
	...
	if (theApp.emuledlg)
		theApp.emuledlg->AddPhoenixText(TBN_LOG, uFlags, szLogLine); //<<-- [TPT] - enkeyDEV(th1) -notifier-
#ifdef _DEBUG
	else{
		TRACE(_T("App Log: %s\n"), szLogLine);
	}
...
void PXLogV(UINT uFlags, LPCTSTR pszFmt, va_list argp)
{
	AddPhoenixTextV(uFlags, DLP_DEFAULT, pszFmt, argp);
}

void PhoenixLog(UINT uFlags, LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(uFlags | LOG_DEBUG, pszFmt, argp);
	va_end(argp);
}

void PhoenixLogError(UINT uFlags, LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(uFlags | LOG_DEBUG | LOG_ERROR, pszFmt, argp);
	va_end(argp);
}

void PhoenixLogWarning(UINT uFlags, LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(uFlags | LOG_DEBUG | LOG_WARNING, pszFmt, argp);
	va_end(argp);
}

void PhoenixLog(LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(LOG_DEBUG, pszFmt, argp);
	va_end(argp);
}

void PhoenixLogError(LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(LOG_DEBUG | LOG_ERROR, pszFmt, argp);
	va_end(argp);
}

void PhoenixLogWarning(LPCTSTR pszFmt, ...)
{
	va_list argp;
	va_start(argp, pszFmt);	
	PXLogV(LOG_DEBUG | LOG_WARNING, pszFmt, argp);
	va_end(argp);
}
// [TPT] - Debug log
...
void AddLogTextV(UINT uFlags, EDebugLogPriority dlpPriority, LPCTSTR pszLine, va_list argptr)
{
	...
	if (theApp.emuledlg)
		theApp.emuledlg->AddLogText(TBN_LOG, uFlags, szLogLine); //<<-- [TPT] - enkeyDEV(th1) -notifier-
#ifdef _DEBUG
	else{
		TRACE(_T("App Log: %s\n"), szLogLine);
	}
