...
void CRichEditCtrlX::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	if (!bReadOnly){
		pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_UNDO, GetResString(IDS_UNDO)));		
		pMenu->AppendSeparator();
	}
	if (!bReadOnly)
		pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_CUT, GetResString(IDS_CUT)));				
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_COPYSELECTED, GetResString(IDS_COPY), theApp.LoadIcon(_T("copy"), 16, 16)));	
	if (!bReadOnly){
		pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_PASTE, GetResString(IDS_PASTE)));				
		pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_REMOVESELECTED, GetResString(IDS_DELETESELECTED)));				
	}
	pMenu->AppendSeparator();
	pMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_SELECTALL, GetResString(IDS_SELECTALL)));

	pMenu->EnableMenuItem(MP_UNDO, CanUndo() ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_CUT, iSelEnd > iSelStart ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_COPYSELECTED, iSelEnd > iSelStart ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_PASTE, CanPaste() ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_REMOVESELECTED, iSelEnd > iSelStart ? MF_ENABLED : MF_GRAYED);
	pMenu->EnableMenuItem(MP_SELECTALL, iTextLen > 0 ? MF_ENABLED : MF_GRAYED);
