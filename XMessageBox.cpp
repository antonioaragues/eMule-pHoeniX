	...
	m_bRightJustifyButtons(pXMB->dwOptions & XMSGBOXPARAMS::RightJustifyButtons)
{
	...
	MEMZERO(m_lpszMessage, MessageSize);

	m_lpszCaption = new TCHAR [MessageSize];
	if (m_lpszCaption)
		m_lpszCaption[0] = _T('\0');
	MEMZERO(m_lpszCaption, MessageSize);
	...
	MEMZERO(m_szCustomButtons, sizeof(m_szCustomButtons));
	...
	MEMZERO(szReport, sizeof(szReport));
...
int CXDialogTemplate::Display()
{
	...
	MEMCOPY(pdest, &m_dlgTempl, sizeof(DLGTEMPLATE));
	...
	*(WORD*)(pdest + 1) = 0; // use default window class
	...
	MEMZERO(pchCaption, nTitleLen*2 + 2);


	MEMCOPY(pchCaption, szTitle, nTitleLen * sizeof(TCHAR));
	...
	for (i = 0; i < m_dlgTempl.cdit; i++)
	{
		...
		MEMCOPY(pdest, (void *)&m_pDlgItemArray[i]->m_dlgItemTemplate,
			sizeof(DLGITEMTEMPLATE));
		...
		*(WORD*)pdest = (WORD)m_pDlgItemArray[i]->m_controltype;	// atom value for the control
		...
		MEMZERO(pchCaption, nChars*sizeof(TCHAR) + 2);
		MEMCOPY(pchCaption, m_pDlgItemArray[i]->m_szCaption, nChars * sizeof(TCHAR));
...
static DWORD ReadRegistry(LPCTSTR lpszCompanyName, LPCTSTR lpszKey)
{
	...
	MEMZERO(szKey, _MAX_PATH*2*sizeof(TCHAR));
	...
	MEMZERO(szPathName, _MAX_PATH*2*sizeof(TCHAR));
