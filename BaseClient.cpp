...
#include "SharedFilesWnd.h"	// [TPT] - SLUGFILLER: showComments
#include "ChatWnd.h"
...
uint32 CUpDownClient::m_upStopReason[2][CUpDownClient::USR_SNAFU+1]; // [TPT] - added snafu
...
CUpDownClient::CUpDownClient(CPartFile* in_reqfile, uint16 in_port, uint32 in_userid,uint32 in_serverip, uint16 in_serverport, bool ed2kID)
{
	...
	m_dwUserCountryIP = ipstr(m_nConnectIP); // [TPT] - IP Country
}

void CUpDownClient::Init()
{
	credits = 0;	
	//m_nSumForAvgUpDataRate = 0; // [TPT] - Maella
	m_bAddNextConnect = false;
	m_nChatstate = MS_NONE;
	m_nKadState = KS_NONE;
	//m_cShowDR = 0; // [TPT] - Maella
	m_nUDPPort = 0;
	m_nKadPort = 0;
	m_nTransferredUp = 0;
	m_cAsked = 0;
	...
	m_nUpDatarate = 0;
	m_pszUsername = 0;
	m_dwUserCountryIP = ipstr(0); // [TPT] - IP Country
	m_nUserIDHybrid = 0;
	m_dwServerIP = 0;
	m_nServerPort = 0;
	...
	m_nDownDatarate = 0;
	//m_nDownDataRateMS = 0; // [TPT] - Maella
	...
	m_strClientSoftware = _T(""); // [TPT] - Patch
	m_strModVersion = _T(""); // [TPT]- modID
	...
	m_bGoofy = false;
	m_strSnafuTag = NULL;
	m_nTrustedtransfer=0;
	m_nTrust=2;		
	m_nNameCRC=0;
	m_bSnafuExclude = false;	
	m_bBan = false;
	m_nBanTimer = 0;
	m_pMod = &theApp.wombatlist->m_strEmptyMod;
	//>>> [TPT] - eWombat SNAFU v2
	m_bpHoeniXClient = false; // [TPT] - Show pHoeniX icon on client detect
	...
	//[TPT] - from here to final, added by TPT (Init())
	toRemove = false; // [TPT] - MUS
	m_dwShieldTime = 0; // MinToMB
	m_abyUpPartStatusHidden = 0; // [TPT] - Powershare
	// [TPT] - enkeyDev: ICS
	m_abyIncPartStatus = 0;
	m_incompletepartVer = 0;
    // [TPT] - enkeyDev: ICS
	// [TPT] - netfinity: Anti HideOS
	m_abySeenPartStatus = 0;
	// [TPT] - netfinity: Anti HideOS
	// [TPT] - WebCache
	// yonatan http start
	m_bProxy = false;
	...
	m_detailDialogInterface = new CClientDetailDialogInterface(this);	// [TPT] - SLUGFILLER: modelessDialogs
}
...
CUpDownClient::~CUpDownClient(){
	...
	// [TPT] - Maella -Downloading source list-
	if(reqfile != NULL){		
		reqfile->RemoveDownloadingSource(this); // => security
	}
	// Maella end
	...
	m_DownloadBlocks_list.RemoveAll(); // [TPT] - Patch
	
	for (POSITION pos = m_RequestedFiles_list.GetHeadPosition();pos != 0;)
		delete m_RequestedFiles_list.GetNext(pos);
	m_RequestedFiles_list.RemoveAll(); // [TPT] - Patch
	...
	m_listFiles.RemoveAll(); // [TPT] - Patch
	// [TPT] - itsonlyme: viewSharedFiles

	// [TPT] - Remove part status
...
void CUpDownClient::ClearHelloProperties()
{
	...
	m_fSharedDirectories = 0;
	m_bMultiPacket = 0;
	m_strModVersion.Empty(); // [TPT] - modID
	m_fPeerCache = 0;
	m_uPeerCacheDownloadPushId = 0;
	m_uPeerCacheUploadPushId = 0;
	m_byKadVersion = 0;
	m_bpHoeniXClient = false;// [TPT] - Show pHoeniX icon on client detect
	m_incompletepartVer = 0; // [TPT] - enkeyDev: ICS
}
...
bool CUpDownClient::ProcessHelloTypePacket(CSafeMemFile* data)
{
	...
	m_bIsHybrid = false;
	m_bIsML = false;
	//<<< eWombat [TPT][SNAFU] safe old values
	eSnafuReason oldsnafu=m_nSnafuReason;
	//>>> eWombat [TPT][SNAFU]
	m_fNoViewSharedFiles = 0;
	m_bUnicodeSupport = false;
	...
	for (uint32 i = 0;i < tagcount; i++){
		...
		switch (temptag.GetNameID()){
			// [TPT] - Maella -AntiCrash/AntiFake handling- (Vorlost/Mortillo)
			case CT_NAME:
				if (m_pszUsername){
					free(m_pszUsername);
				...
				if(((LPCTSTR)temptag.GetStr()) == NULL)
				{
					...
					TCHAR buffer[100];
					buffer[0] = '\0';
					for(uint16 i=0; i<16; i++) _stprintf(buffer, _T("%s%02X"), buffer, (uchar)m_achUserHash[i]);
					AddLogLine(true,_T("A client tries to Crash eMule with an invalid Username !!! Userhash: %s, Userport: %d, we name him now:!!!CrashName!!!"), buffer, m_nUserPort );
					
					SetUserName(_T("!!!CrashName!!!"));	// [TPT] - itsonlyme: viewSharedFiles		
				}
				else if (temptag.GetStr().Compare(_T("")) != 0)
				{
					//<<< [TPT] - eWombat SNAFU v2
					if (thePrefs.GetAntiSnafu() || !IsSnafu())
					{
						ULONG crc=SnafuNameCRC(temptag.GetStr());
						if (m_nNameCRC!=0 && m_nNameCRC != crc)
							DoSnafu(SNAFU_NAME,true,false);
						SnafuCheckNick(temptag.GetStr());
						m_nNameCRC = crc;				
					}
					//>>> [TPT] - eWombat SNAFU v2

					SetUserName(temptag.GetStr());	// [TPT] - itsonlyme: viewSharedFiles
			...
			case CT_PORT:
				...
				if (nUserPort != temptag.GetInt())
				{
					...
					if (thePrefs.GetAntiSnafu() && !IsSnafu())
					{
						if (thePrefs.GetVerbose()) PhoenixLogWarning(_T("User %s tries to crash eMule with an invalid Userport: %d but the real one is %d, corrected !!!"),m_pszUsername,temptag.GetInt(),nUserPort );
						DoSnafu(SNAFU_ACTION,true,false);
					}
					//>>> [TPT] - eWombat SNAFU v2
				}
				else
			...
			case CT_MOD_VERSION:
				if (temptag.IsStr())
				{
					...
					m_bpHoeniXClient = (StrStrI(m_strModVersion, _T("pHoeniX")) != NULL); // [TPT] - Show pHoeniX icon on client detect
				}
				...
				break;
			//<<< [TPT] - eWombat SNAFU v2
			case CT_FRIENDSHARING: //Friendshare Client is Possible Leecher
				if (thePrefs.GetAntiFriendshare()==true)
				{
					if (temptag.GetInt() == FRIENDSHARING_ID) //Mit dieser ID Definitiv
	...
	socket->GetPeerName((SOCKADDR*)&sockAddr, &nSockAddrLen);
	SetIP(sockAddr.sin_addr.S_un.S_addr);
	m_dwUserCountryIP = ipstr(m_dwUserIP); // [TPT] - IP Country
	...
	bool bHashChanged = false; // [TPT] - eWombat SNAFU v2
	...
	if (credits == NULL){
		...
		if (!theApp.clientlist->ComparePriorUserhash(m_dwUserIP, m_nUserPort, pFoundCredits)){
			if (thePrefs.GetAntiSnafu())
			{
				// [TPT] - eWombat SNAFU v2
				if (!IsSnafu())
					m_dwLastSignatureIP = 0; //Force to recheck Sig
				DoSnafu(SNAFU_HASHCHG,false,false);
				// [TPT] - eWombat SNAFU v2
			}
			else
			{
		...
		credits = pFoundCredits;
		bHashChanged=true;// [TPT] - eWombat SNAFU v2
		if (!thePrefs.GetAntiSnafu())
		{
			if (thePrefs.GetLogBannedClients())
	...
	//<<< [TPT] - eWombat SNAFU v2Check for hash changers
	if (thePrefs.GetAntiSnafu() && !IsSnafu())
	{
		//if client is using my Hash 
		if(theApp.serverconnect->GetClientID()!= m_nUserIDHybrid && md4cmp(m_achUserHash, thePrefs.GetUserHash())==0)		
			// [TPT] - eWombat SNAFU v2
			DoSnafu(SNAFU_MYHASH,false,false);	
			// [TPT] - eWombat SNAFU v2
	}
	//>>> [TPT] - eWombat SNAFU v2
	...
	// [TPT] - eWombat SNAFU v2
	if (thePrefs.GetAntiSnafu() && oldsnafu!=m_nSnafuReason)
		DoSnafuInfo();
	// [TPT] - eWombat SNAFU v2
...
void CUpDownClient::SendMuleInfoPacket(bool bAnswer){
	...
	//[TPT] - Added by SiRoB, Don't send MOD_VERSION to client that don't support it to reduce overhead	
	...
	//[TPT] END   - Added by SiRoB, Don't send MOD_VERSION to client that don't support it to reduce overhead
	...
	CTag tag4(ET_SOURCEEXCHANGE,thePrefs.GetDisabledXS() ? 0 : 3); // [TPT] - Maella -Enable/Disable source exchange in preference-
	tag4.WriteTagToFile(&data);
	CTag tag5(ET_COMMENTS,2); // [TPT] - SLUGFILLER: showComments
...
void CUpDownClient::ProcessMuleInfoPacket(char* pachPacket, uint32 nSize)
{
	...
	CSafeMemFile data((BYTE*)pachPacket,nSize);
	m_byCompatibleClient = 0;
	eSnafuReason oldsnafu=m_nSnafuReason; // [TPT] - eWombat SNAFU v2
		...
		switch (temptag.GetNameID()){
			...
			case ET_MOD_VERSION:
				if (temptag.IsStr())
				{
					...
					m_bpHoeniXClient = (StrStrI(m_strModVersion, _T("pHoeniX")) != NULL); 	// [TPT] - Show pHoeniX icon on client detect
				// [TPT] - eWombat SNAFU v2
					SnafuCheckMod(m_strModVersion);
					try {
						if (m_bGoofy)
							m_pMod = theApp.wombatlist->GetModPtr(m_strModVersion);
					} catch(...)
					{
						if (thePrefs.GetVerbose()) 
							PhoenixLogWarning(_T("exception in ET_MOD_VERSION"));
						m_pMod=&theApp.wombatlist->m_strEmptyMod;
					}
					// [TPT] - eWombat SNAFU v2
				}
...
void CUpDownClient::SendHelloTypePacket(CSafeMemFile* data)
{
	...
	data->WriteUInt32(theApp.GetID()); // [TPT]
...
//					( RESERVED												)
	...
	const UINT uAcceptCommentVer	= 2; // [TPT] - SLUGFILLER: showComments
...
//				(RESERVED			     ) 
	...
	if (bSendModVersion || m_clientSoft == SO_LPHANT)
	{
		...
		for(int i=0; localMasterKeyNeeded && i < WC_KEYLENGTH; i++)
			localMasterKeyNeeded = (Crypt.localMasterKey[i]==0);//[TPT] - Webcache 1.9 beta3
...
// [TPT] - SLUGFILLER: showComments - needed complete revision
void CUpDownClient::ProcessMuleCommentPacket(char* pachPacket, uint32 nSize){	
	...
	if (file == reqfile)
		cs.m_strFileName = m_strClientFilename;
	else
		cs.m_strFileName = "-";
	...
	if (uLength > 0){
		cs.m_strComment = data.ReadString(GetUnicodeSupport(), uLength);
	...
	cs.m_iUserIP = GetIP();
	cs.m_iUserPort = GetUserPort();
	cs.m_strUserName = m_pszUsername;
	file->AddComment(cs);
}
// [TPT] - SLUGFILLER: showComments

bool CUpDownClient::Disconnected(LPCTSTR pszReason, bool bFromSocket, UpStopReason reason) // [TPT] - Maella -Upload Stop Reason-
{
	...
	if (GetUploadState() == US_UPLOADING || GetUploadState() == US_CONNECTING)
	{
		...
		theApp.uploadqueue->RemoveFromUploadQueue(this, pszReason, reason); // [TPT] - Maella -Upload Stop Reason-
	}
	...
	if (GetDownloadState() == DS_DOWNLOADING){
		...
		{
			case USR_CANCELLED:
				SetDownloadState(DS_ONQUEUE, CString(_T("Disconnected: ")) + pszReason, DSR_CANCELLED); // [TPT] - Maella -Download Stop Reason-
				break;
			case USR_TIMEOVER:
				SetDownloadState(DS_ONQUEUE, CString(_T("Disconnected: ")) + pszReason, DSR_TIMEOUT); // [TPT] - Maella -Download Stop Reason-
				break;
			case USR_EXCEPTION:
				SetDownloadState(DS_ONQUEUE, CString(_T("Disconnected: ")) + pszReason, DSR_EXCEPTION); // [TPT] - Maella -Download Stop Reason-
				break;			
			default:
				SetDownloadState(DS_ONQUEUE, CString(_T("Disconnected: ")) + pszReason, DSR_NONE); // [TPT] - Maella -Download Stop Reason-
				break;
		}
	}
	else{
		...
		if(GetDownloadState() == DS_CONNECTED){
		    //[TPT] - MORPH START - Added by SiRoB, Don't kill source if it's the only one complet source, it's a friend or a proxy
			if(reqfile && m_bCompleteSource && reqfile->m_nCompleteSourcesCountLo == 1  || IsFriend() || IsProxy())
				SetDownloadState(DS_ONQUEUE);
			else
			//[TPT] - MORPH END   - Added by SiRoB, Don't kill source if it's the only one complet source or it's a friend
	...
	switch(m_nUploadState){
		...
		case US_WAITCALLBACK:
			//[TPT] - MORPH START - Added by SiRoB, Don't kill client if we are the only one complet source or it's a friend or it's a proxy.
			...
			//[TPT] - MORPH END   - Added by SiRoB, Don't kill client if we are the only one complet source or it's a friend or it's a proxy.
		case US_ERROR:
			theApp.clientlist->m_globDeadSourceList.AddDeadSource(this);
			bDelete = true;
	}
	switch(m_nDownloadState){
		...
		case DS_WAITCALLBACK:
			...
			//[TPT] - MORPH END   - Added by SiRoB, Don't kill source if it's a proxy
		case DS_ERROR:
			theApp.clientlist->m_globDeadSourceList.AddDeadSource(this);
			bDelete = true;
	}
	...
	// [TPT] - SLUGFILLER: modelessDialogs - keep client object, otherwise the dialog would disappear
	...
	socket = 0;
    if (m_iFileListRequested || m_bRequestingFileList){	// [TPT] - itsonlyme: viewSharedFiles
		LogWarning(LOG_STATUSBAR, GetResString(IDS_SHAREDFILES_FAILED), GetUserName());
        m_iFileListRequested = 0;
		m_bRequestingFileList = false;	// [TPT] - itsonlyme: viewSharedFiles
	}
	if (m_Friend)
	...
	if (uClientIP)
	{
		...
		if (theApp.ipfilter->IsFiltered(uClientIP))
		{
			...
			if (thePrefs.GetLogFilteredIPs())
			{
				...
				buffer[0] = '\0';
				for(uint16 i=0; i<16; i++) _stprintf(buffer, _T("%s%02X"), buffer, (uchar)m_achUserHash[i]);
				AddDebugLogLine(true, GetResString(IDS_IPFILTERED), ipstr(uClientIP), theApp.ipfilter->GetLastHit(), buffer, m_pszUsername);
				// [TPT]
			}
		...
		if (theApp.clientlist->IsSnafuClient(uClientIP)) // [TPT] - eWombat [SNAFU]
		{
			if (thePrefs.GetLogBannedClients())
				AddDebugLogLine(false, _T("Refused to connect to banned client %s"), DbgGetClientInfo());
			if (Disconnected(_T("Banned IP"), false, CUpDownClient::USR_SNAFU)) // [TPT] - Maella -Upload Stop Reason-
			{
				delete this;
				return false;
			}
...
void CUpDownClient::ConnectionEstablished()
{
	...
	if (m_bRequestingFileList)	// [TPT] - itsonlyme: viewSharedFiles	
	{
		// [TPT] - itsonlyme: viewSharedFiles
		m_bRequestingFileList = false;
...
// [TPT] - itsonlyme: viewSharedFiles
void CUpDownClient::RequestSharedFileList(bool bForce){
...
void CUpDownClient::ProcessSharedFileList(char* pachPacket, uint32 nSize, LPCTSTR pszDirectory){
	if (m_iFileListRequested > 0)
	{
		...
		CSafeMemFile packet((BYTE*)pachPacket,nSize);
		UINT results = packet.ReadUInt32();

		for (UINT i = 0; i < results; i++){
			CSearchFile* toadd = new CSearchFile(&packet, GetUnicodeSupport(), nSearchID, 0, 0, dir);
...
void CUpDownClient::ProcessSharedDirsList(char* pachPacket, uint32 nSize)
{
	bool requested;
	if (m_iFileListRequested > 0)
	{
        m_iFileListRequested--;
		CSafeMemFile data((uchar*)pachPacket, nSize);		
		UINT uDirs = data.ReadUInt32();
		if (uDirs > 0)
			m_listDirs.RemoveAll();
		for (UINT i = 0; i < uDirs; i++)
		{
			CString strDir = data.ReadString(GetUnicodeSupport());
			if (thePrefs.GetVerbose())
...
void CUpDownClient::ProcessSignaturePacket(uchar* pachPacket, uint32 nSize){
	...
	if (theApp.clientcredits->VerifyIdent(credits, pachPacket+1, pachPacket[0], GetIP(), byChaIPKind ) ){
		...
		//<<< eWombat [SUI][TPT]
		if (IsSnafu())		//<<< [TPT] - eWombat SNAFU v2
		{
			if (m_nSnafuReason==SNAFU_UNKNOWN ||
				m_nSnafuReason==SNAFU_NAME	  ||
				m_nSnafuReason==SNAFU_HASHCHG ||
				m_nSnafuReason==SNAFU_MODSTR  ||
				m_nSnafuReason==SNAFU_NICK	 )
				UnSnafu();
		}
		if (m_nTrust < 4)
			m_nTrust = 4;	//>>> eWombat [SNAFU]
		ResetSUITime();
		//>>> eWombat [SUI]
...
void CUpDownClient::InfoPacketsReceived(){
	...
	m_byInfopacketsReceived = IP_NONE;
	// [TPT] - eWombat SNAFU v2
	if (m_nSnafuReason==SNAFU_MYHASH || m_nSnafuReason==SNAFU_CLIENT)
		return;
...
void CUpDownClient::ResetFileStatusInfo()
{
	...
	// [TPT] - Official patch. MORPH START - Added by SiRoB, HotFix Due Complete Source Feature
	m_nUpCompleteSourcesCount = 0;
	//MORPH END  - Added by SiRoB, HotFix Due Complete Source Feature
}

// [TPT] - eWombat SNAFU v2 - Moved due Snafu
/*
...
void CUpDownClient::AssertValid() const
{
	...
	//(void)s_UpStatusBar; // [TPT] - Maella
	(void)requpfileid;
    (void)m_lastRefreshedULDisplay;
	//m_AvarageUDR_list.AssertValid(); // [TPT]
	...
	//(void)m_nDownDataRateMS; // [TPT]
	//(void)m_nSumForAvgDownDataRate; // [TPT]
	//(void)m_cShowDR; // [TPT]
...
CString CUpDownClient::GiveMetheQRString() const
{
	CString state;
	uint16 nRemoteQueueRank = GetRemoteQueueRank();
	if (nRemoteQueueRank)
	{
		uint16 nDifference =  GetDifference();
		if (nDifference==nRemoteQueueRank)
		{
			state.Format(_T("QR: %u (%u)"),nRemoteQueueRank,(nRemoteQueueRank-nDifference));
		}      
		else if (nDifference<nRemoteQueueRank)
		{  
			state.Format(_T("QR: %u (+%u)"),nRemoteQueueRank,(nDifference));
			if (nDifference==0)
			{
				state.Format(_T("QR: %u (%u)"),nRemoteQueueRank,(nDifference));
			}
		...
		{
			state.Format(_T("QR: %u (-%u)"),nRemoteQueueRank,(65536-nDifference));
		}
	}
	else
	{
		state = GetResString(IDS_ONQUEUE);
	}
...
CString CUpDownClient::GetUploadStateDisplayString() const
{
	...
	switch (GetUploadState()){
		...
		case US_WAITCALLBACK:
			...
			break;
		case US_UPLOADING:
			strState = GetResString(IDS_TRANSFERRING); // [TPT] - Remove trickling info
...
{
	if(m_pszUsername)
		info.Format(GetResString(IDS_TT_USERINFO), m_pszUsername, m_nUserIDHybrid, GetResString(HasLowID() ? IDS_IDLOW : IDS_IDHIGH), DbgGetFullClientSoftVer());
	else
		info = _T("<t=2><b>?</b><br><br><hr=100%%><br>");	
	if (m_dwServerIP)
	{
		in_addr server;
		server.S_un.S_addr = m_dwServerIP;
		info.AppendFormat(GetResString(IDS_TT_SERVER), ipstr(server), m_nServerPort);
	}
}
...
void CUpDownClient::GetTooltipDownloadInfo(CString &info, bool a4af, CPartFile* file, bool base) 
{
	if (IsEd2kClient())
	{
		...
		if ((thePrefs.GetVerbose()) && (thePrefs.GetBlockPhoenixMsg() == false))
		{
			info.AppendFormat(_T("<b>Last ask to this client:<t=2></b>%s<br>"), 
				CastSecondsToHM((::GetTickCount() - (DWORD)this->GetLastAskedTime())/1000));
			info.AppendFormat(_T("<b>Next TCP ask to this client:<t=2></b>%s<br>"), 
				CastSecondsToHM(((DWORD)this->GetNextTCPAskedTime() - ::GetTickCount())/1000));
		}
		info.AppendFormat(_T("<b>Next ask to this client:<t=2></b>%s (%s)<br>"), 
			CastSecondsToHM(this->GetTimeUntilReask(this->reqfile)/1000),
			CastSecondsToHM(this->GetTimeUntilReask(file)/1000)); // ZZ:DownloadManager
		info.AppendFormat(GetResString(IDS_TT_SOURCEINFO), GetAskedCountDown());
		info.AppendFormat(GetResString(IDS_TT_SOURCEINFO2), GetAvailablePartCount());
		if (reqfile)
		{
			if (!a4af) // normal client
			{
				if (reqfile->GetFileName())
					info.AppendFormat(GetResString(IDS_TT_CLIENTSOURCENAME), reqfile->GetFileName());
				
				// [TPT] - SLUGFILLER: showComments adapted by TPT
				...
				{					
					...
					if (cur_cs.m_iUserIP == this->GetIP() && cur_cs.m_iUserPort == this->GetUserPort())
					{
						if (cur_cs.m_strComment.IsEmpty())
							info.AppendFormat(GetResString(IDS_TT_CMT_NONE)); 	//No comment entered
						else
							info.AppendFormat(GetResString(IDS_TT_CMT_READ), cur_cs.m_strComment);

						info.AppendFormat(GetResString(IDS_TT_CMT_RATING), GetRateString(cur_cs.m_uRating));
						break;
					} 					
				}
				// [TPT] - SLUGFILLER: showComments adapted by TPT					
				...
				info.AppendFormat(GetResString(IDS_TT_ASKEDFAF), reqfile->GetFileName());
		}
	// ZZ:DownloadManager -->
        try {
            if (thePrefs.IsExtControlsEnabled() && !this->m_OtherRequests_list.IsEmpty()){
                CString a4afStr;
                a4afStr.AppendFormat(_T("<br><br><b>A4AF files:</b><br>"));
                bool first = TRUE;
                for (POSITION pos3 = this->m_OtherRequests_list.GetHeadPosition(); pos3!=NULL; this->m_OtherRequests_list.GetNext(pos3)){
                    if(!first) {
                        a4afStr.Append(_T("<br>"));
                    }
                    first = FALSE;
                    a4afStr.AppendFormat(_T("%s"),this->m_OtherRequests_list.GetAt(pos3)->GetFileName());
                };
                info += a4afStr;
            }
        }catch (...){
...
// ZZ:DownloadManager <--
	...
	{
		info.AppendFormat(_T("<b>URL:<t=2></b>%s<br><b>Available parts:<t=2></b>%u<br>"), this->GetUserName(), this->GetAvailablePartCount());
	}
}
...
void CUpDownClient::GetTooltipUploadInfo(CString &info, bool base)
{
	...
	{
		info.AppendFormat(GetResString(IDS_TT_REQUESTED), file->GetFileName());
		info.AppendFormat(GetResString(IDS_TT_FILESTATS_SESSION), file->statistic.GetAccepts(), file->statistic.GetRequests(), CastItoXBytes(file->statistic.GetTransferred(), false, false));
		info.AppendFormat(GetResString(IDS_TT_FILESTATS_TOTAL), file->statistic.GetAllTimeAccepts(), file->statistic.GetAllTimeRequests(), CastItoXBytes(file->statistic.GetAllTimeTransferred(), false, false));
	}
	else
...
void CUpDownClient::GetTooltipQueueInfo(CString &info, bool base)
{
	...
	info.AppendFormat(GetResString(IDS_TT_QL_RATING), GetScore(false,false,true));
	info.AppendFormat(GetResString(IDS_TT_QL_SCORE), GetScore(false));
	info.AppendFormat(GetResString(IDS_TT_SOURCEINFO), GetAskedCount());
	info.AppendFormat(GetResString(IDS_TT_QL_LASTSEEN), CastSecondsToHM((GetTickCount() - GetLastUpRequest())/1000));
	info.AppendFormat(GetResString(IDS_TT_ENTEREDQUEUE), CastSecondsToHM((GetTickCount() - GetWaitStartTime())/1000));
}
...
void CUpDownClient::GetTooltipClientInfo(CString &info)
{
	...
	if(reqfile)
	{
		info.AppendFormat(GetResString(IDS_TT_DOWNLOAD_INFO));
		info.AppendFormat(GetResString(IDS_TT_REQUESTED), reqfile->GetFileName());
	...
	{
		case US_UPLOADING:
			info.AppendFormat(GetResString(IDS_TT_UPLOAD_INFO));
			GetTooltipQueueInfo(info, false);
			break;
		case US_ONUPLOADQUEUE:
			info.AppendFormat(GetResString(IDS_TT_QUEUE_INFO));
