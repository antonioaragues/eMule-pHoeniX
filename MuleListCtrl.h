...
class CMuleListCtrl : public CListCtrl
{
	...
	friend class CSelNAFCDlg; // [TPT] - NAFC Selection

public:
	CMuleListCtrl(PFNLVCOMPARE pfnCompare = SortProc, DWORD dwParamSort = 0);
	virtual ~CMuleListCtrl();

	int GetItemUnderMouse(CPoint* pt = NULL);// [TPT] - MFCK [addon] - New Tooltips [Rayita]
...
protected:
	...
	CList<DWORD>     m_dwParamSort;	// [TPT] - SLUGFILLER: multiSort
	COLORREF         m_crWindow;
