...
void CPPgScheduler::OnNMRclickActionlist(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	pActionMenu->AddSideBar(new CMenuXPSideBar(17, _T("")));
	pActionMenu->SetSideBarStartColor(RGB(255,0,0));
	pActionMenu->SetSideBarEndColor(RGB(255,128,0));
	pActionMenu->SetSelectedBarColor(RGB(242,120,114));
	
	CMenuXP *pActionSel = new CMenuXP;
	pActionSel->CreatePopupMenu();
	pActionSel->SetMenuStyle(CMenuXP::STYLE_STARTMENU);
	pActionSel->SetSelectedBarColor(RGB(242,120,114));

	CMenuXP *pCatActionSel = new CMenuXP;
	pCatActionSel->CreatePopupMenu();
	pCatActionSel->SetMenuStyle(CMenuXP::STYLE_STARTMENU);
	pCatActionSel->SetSelectedBarColor(RGB(242,120,114));
	...
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_SETUPL,GetResString(IDS_PW_UPL)));	
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_SETDOWNL,GetResString(IDS_PW_DOWNL)));
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_SOURCESL,GetResString(IDS_LIMITSOURCES)));
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_CON5SEC,GetResString(IDS_LIMITCONS5SEC)));
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_CONS,GetResString(IDS_PW_MAXC)));
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_CATSTOP,GetResString(IDS_SCHED_CATSTOP)));
	pActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+ACTION_CATRESUME,GetResString(IDS_SCHED_CATRESUME)));

	pActionMenu->AppendODPopup(MF_STRING | MF_POPUP, pActionSel, new CMenuXPText(0, GetResString(IDS_ADD)));

	if (isCatAction) {
		if (thePrefs.GetCatCount()>1) 
			pCatActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+20,GetResString(IDS_ALLUNASSIGNED)));			
		pCatActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+21,GetResString(IDS_ALL)));		
		for (int i=1;i<thePrefs.GetCatCount();i++)
			pCatActionSel->AppendODMenu(MF_STRING,new CMenuXPText(MP_SCHACTIONS+22+i,thePrefs.GetCategory(i)->title));			
		pActionMenu->AppendODPopup(MF_STRING | MF_POPUP, pCatActionSel, new CMenuXPText(0, GetResString(IDS_SELECTCAT)));		
	} else
		pActionMenu->AppendODMenu(MF_STRING | nFlag,new CMenuXPText(MP_CAT_EDIT,GetResString(IDS_EDIT)));		

	pActionMenu->AppendODMenu(MF_STRING | nFlag,new CMenuXPText(MP_CAT_REMOVE,GetResString(IDS_REMOVE)));		

	pActionMenu->TrackPopupMenu(TPM_LEFTALIGN |TPM_RIGHTBUTTON, point.x, point.y, this);

	delete pActionSel;
	delete pCatActionSel;
	delete pActionMenu;
