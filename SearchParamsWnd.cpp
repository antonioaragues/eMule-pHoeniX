...
void CSearchParamsWnd::UpdateControls()
{
	...
	*/
	m_ctlOpts.SetItemData(orAvailability, (iMethod==SearchTypeFileDonkey) ? 1 : 0);
	m_ctlOpts.SetItemData(orExtension, (iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orCompleteSources, (iMethod==SearchTypeKademlia || iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orCodec, (iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orBitrate, (iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orLength, (iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orTitle, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal || iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orAlbum, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal || iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
	m_ctlOpts.SetItemData(orArtist, (iMethod==SearchTypeEd2kServer || iMethod==SearchTypeEd2kGlobal || iMethod==SearchTypeFileDonkey || iMethod==SearchTypeEmugle) ? 1 : 0);
...
void CSearchParamsWnd::SetAllIcons()
{
	...
	// [TPT] - [MoNKi: -eMugle.com as search method-]
	iml.Add(CTempIconLoader(_T("SearchMethod_EMUGLE"), 16, 16));
	// End -eMugle.com as search method-
	...
	iml.Add(CTempIconLoader(_T("bticon"), 16, 16));
...
void CSearchParamsWnd::InitMethodsCtrl()
{
	...
	// [TPT] - [MoNKi: -eMugle.com as search method-]
	VERIFY( m_ctlMethod.AddItem(_T("eMugle (Web)"), 4) == SearchTypeEmugle );
	// End -eMugle.com as search method-
...
void CSearchParamsWnd::InitFileTypesCtrl()
{
	...
	//[TPT] - Search torrents
	lstFileTypeCbEntries.push_back(SFileTypeCbEntry(GetResString(IDS_SEARCH_TORRENT), ED2KFTSTR_TORRENT, 8));
...
void CSearchParamsWnd::UpdateUnicodeCtrl()
{
	...
	{
	*/
	ESearchType curSel = (ESearchType)m_ctlMethod.GetCurSel();
	if (curSel != SearchTypeFileDonkey && curSel != SearchTypeEmugle)
	{
	// End -eMugle.com as search method-
		CString strExpr;
