...
#define TBN_KILLTIMER 	if (m_nActiveTimer != 0) {timeKillEvent(m_nActiveTimer); timeEndPeriod(m_dwTimerPrecision); m_nActiveTimer=0;}
#define TBN_SETTIMER(event,delay,precision)  m_nAnimStatus=event; if (m_nActiveTimer==0) {timeBeginPeriod(precision); m_nActiveTimer=timeSetEvent(delay, precision, ThreadTimerFunc, DWORD(this), TIME_PERIODIC);}
...
CTaskbarNotifier::CTaskbarNotifier()
{
	...
	// If running on NT, timer precision is 20 ms, if not timer precision is 50 ms
	OSVERSIONINFO osvi;
	osvi.dwOSVersionInfoSize=sizeof(OSVERSIONINFO);
	GetVersionEx(&osvi);
	if (osvi.dwPlatformId==VER_PLATFORM_WIN32_NT)
		m_dwTimerPrecision=20;
	else
...
CTaskbarNotifier::~CTaskbarNotifier()
{
	...
	m_MessageHistory.RemoveAll(); // [TPT] - Patch memory leak
...
int CTaskbarNotifier::Create(CWnd *pWndParent)
{
	...
	return CreateEx(/*WS_EX_TOPMOST*/WS_EX_TOOLWINDOW, s_szClassName, NULL,WS_POPUP,0,0,0,0,/*pWndParent->m_hWnd*/NULL,NULL); //<<--clicking on tbn will not activate background window
}

BOOL CTaskbarNotifier::LoadConfiguration(CNotifierSkin tbnConfig) {	
	Hide();

	if (!tbnConfig.isValid()) 
		return FALSE;

	if (!SetBitmap(tbnConfig.GetBmpFileName(), GetRValue(tbnConfig.GetBmpTransparentColor()), 
		GetGValue(tbnConfig.GetBmpTransparentColor()), GetBValue(tbnConfig.GetBmpTransparentColor())))
			return FALSE;

	SetTextFont(tbnConfig.GetFontType(),tbnConfig.GetFontSize(),TN_TEXT_NORMAL,TN_TEXT_UNDERLINE);
	SetTextColor(tbnConfig.GetNormalTextColor(),tbnConfig.GetSelectedTextColor());		
	SetTextRect(tbnConfig.GetTextArea());
	SetCloseBtnRect(tbnConfig.GetCloseBtnArea());
	SetHistoryBtnRect(tbnConfig.GetHistoryBtnArea());

	m_dwTimeToShow=tbnConfig.GetTimeToShow();
	m_dwTimeToStay=tbnConfig.GetTimeToStay();
	m_dwTimeToHide=tbnConfig.GetTimeToHide();
	return TRUE;
}
...
void CTaskbarNotifier::SetFramerateDiv(int div) {
	m_nFramerateDiv=div;
}

void CTaskbarNotifier::SetTimeToShow(DWORD timeToShow) {
	m_dwTimeToShow=timeToShow;
}
void CTaskbarNotifier::SetTimeToHide(DWORD timeToHide) {
	m_dwTimeToHide=timeToHide;
}
void CTaskbarNotifier::SetTimeToStay(DWORD timeToStay) {
...
void CTaskbarNotifier::Show(LPCTSTR szCaption, int nMsgType, LPCTSTR pszLink, BOOL bAutoClose)
{
	...
	switch (m_nAnimStatus)
	{
		case IDT_HIDDEN:
			...
			TBN_SETTIMER(IDT_APPEARING,m_dwShowEvents,m_dwTimerPrecision)
			break;
		...
		case IDT_WAITING:
			...
			if (!m_bMouseIsOver) {
				TBN_KILLTIMER			
				TBN_SETTIMER(IDT_WAITING,m_dwTimeToStay,m_dwTimerPrecision)
			}
			break;
...
void CTaskbarNotifier::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_rcCloseBtn.PtInRect(point)) 
	{
			...
			TBN_KILLTIMER
			TBN_SETTIMER(IDT_DISAPPEARING,m_dwHideEvents,m_dwTimerPrecision)
	}
...
BOOL CTaskbarNotifier::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	...
	{
		if (m_rcCloseBtn.PtInRect(mouseposition) ||
			m_rcHistoryBtn.PtInRect(mouseposition) ||
			m_rcText.PtInRect(mouseposition)) {
...
void CTaskbarNotifier::UpdatePos() 
{
	switch (m_nAnimStatus)
	{
		case IDT_APPEARING:
...
void CALLBACK CTaskbarNotifier::ThreadTimerFunc(UINT uTimerID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dw1, DWORD_PTR dw2)
{
	//using multimedia timer will not speed up window refreshing (we are calling the main thread to do all the work)
	//but makes timings more accurate 
	CTaskbarNotifier* tbn = (CTaskbarNotifier*) dwUser;
	CSingleLock(&tbn->m_tbnActionMutex,TRUE);	
	tbn->UpdatePos();
}
