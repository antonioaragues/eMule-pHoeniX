...
#include "EnBitmap.h"
#include <atlimage.h>
#include "CxImage\ximage.h" // [TPT] - Monki Transparency
...
BOOL CEnBitmap::LoadImage(LPCTSTR szImagePath, COLORREF crBack)
{
	...
	// [TPT] - Monki Transparency
	// Check ComCtl32 version, because CImageList don't support alpha channels if
	// ComCtl32 < 6 (WindowsXP)
	if(_AfxGetComCtlVersion() >= MAKELONG(0, 6))
	// [TPT] - Monki Transparency
	{
...
// [TPT] - Monki Transparency
BOOL CEnBitmap::LoadCxImage(LPCTSTR pszImagePath, COLORREF crMask)
{
	CDC dcMem, dcMem2;
	CDC* pDC = CWnd::GetDesktopWindow()->GetDC();
	BOOL bResult = FALSE;
	...
	{
		CxImage xImg, xImg2, xMask;
		if(xImg.Load(pszImagePath))
		{
			CBitmap bmMem, bmMem2;

			if (bmMem.CreateCompatibleBitmap(pDC, xImg.GetWidth(), xImg.GetHeight())
			&& bmMem2.CreateCompatibleBitmap(pDC, xImg.GetWidth(), xImg.GetHeight()))
			{
				CBitmap* pOldBM = dcMem.SelectObject(&bmMem);
				CBitmap* pOldBM2 = dcMem2.SelectObject(&bmMem2);
	   
				//xMask = alpha channel
				if(xImg.AlphaSplit(&xMask))
				{
					xMask.Threshold(0); //This creates a B/W mask instead of a grayscale mask (8 bit mask -> 1 bit Mask)
					xMask.GrayScale(); //CxImage.AlphaSet() needs a grayscale image.

					//Fill dcMem with mask color
					dcMem.FillSolidRect(0, 0, xImg.GetWidth(), xImg.GetHeight(), crMask);
				   
					//Fill dcMem2 with button's color and draw the image with alpha channel
					dcMem2.FillSolidRect(0, 0, xImg.GetWidth(), xImg.GetHeight(), GetSysColor(COLOR_3DFACE));
					xImg.Draw(dcMem2, 0,0);
				   
					if(xImg2.CreateFromHBITMAP(bmMem2))
					{
						//Sets the new alpha channel and draws the new image to dcMem.
						xImg2.AlphaSet(xMask);
						xImg2.Draw(dcMem,0,0);

						//Destroys temp image
						xImg2.Destroy();

						// We need to do this if screen bpp > 24.
						// CreateFromHBITMAP will convert the image from 32bpp (or greater)
						// to 24bpp (because CxImage do not support > 24bpp)
						// For now CxImage do not support images with bpp > 24 and
						// DecreaseBpp(24) is useless, but maybe we need this for future versions.
						if(xImg2.CreateFromHBITMAP(bmMem))
						{
							if(xImg2.GetBpp()>24)
								xImg2.DecreaseBpp(24, TRUE);

							bResult = CBitmap::Attach(xImg2.MakeBitmap());
						}
					}
				}
