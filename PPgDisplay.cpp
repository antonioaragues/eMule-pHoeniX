...
#include "SharedFilesWnd.h"	// [TPT] - itsonlyme: displayOptions
#include "ServerWnd.h"
#include "HelpIDs.h"
...
BOOL CPPgDisplay::OnApply()
{
	...
	//<<< MFCK [addon] - New Tooltips [Rayita]
	theApp.emuledlg->transferwnd->SetTTDelay(); // modified by rayita
	theApp.emuledlg->serverwnd->SetTTDelay(); // added by rayita
	theApp.emuledlg->SetTTDelay(); // added by rayita

	theApp.emuledlg->searchwnd->SetToolTipsDelay(thePrefs.GetToolTipDelay());
	//>>> MFCK [addon] - New Tooltips [Rayita]
	...
	if (!thePrefs.ShowRatesOnTitle()) {
		_stprintf(buffer,_T("eMule v%s"),theApp.m_strCurVersionLong + _T(" ") + MOD_VERSION); // [TPT]- modID
