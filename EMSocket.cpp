...
void CEMSocket::ClearQueues(){
	...
	// [TPT]
	// Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	while(controlpacket_queue.IsEmpty() == false){
		delete controlpacket_queue.RemoveTail();
	}
	controlpacket_queue.RemoveAll();

	while(standartpacket_queue.IsEmpty() == false){
		delete standartpacket_queue.RemoveTail().packet;
	}
	// Maella end
...
void CEMSocket::OnReceive(int nErrorCode){
	...
	// [TPT] - WebCache	
	// yonatan http start
	//Dangerous (and uggly). When a socket receives some raw data (e.g. downloading a movie), you have a 
	//small possiblity that the data contains this value.
	//ikaTODO: need some checks here?
	...
	while ((rend - rptr >= PACKET_HEADER_SIZE) || ((pendingPacket != NULL) && (rend - rptr > 0)))
	{
		...
		if(pendingPacket == NULL){
			...
			switch (pendingPacket->prot){
				...
				case OP_EMULEPROT:
				// [TPT] - WebCache 
				case OP_WEBCACHEPACKEDPROT://[TPT] - Webcache 1.9 beta3
    ...
    while(sentStandardPacketBytesThisCall + sentControlPacketBytesThisCall < maxNumberOfBytesToSend && anErrorHasOccured == false && // don't send more than allowed. Also, there should have been no error in earlier loop
        ...
        while (sent < sendblen &&
		    ...
		    if (result == (uint32)SOCKET_ERROR){
				...
				// [TPT]
				theApp.pBandWidthControl->AddeMuleOutTCPOverall(result);		

                // Log send bytes in correct class
                if(m_currentPacket_is_controlpacket == false) {
					theApp.pBandWidthControl->AddeMuleOut(result);	// [TPT]
 ...
 * @author SlugFiller
 */
//[TPT] - New Parameter from Sirob (morph) to set trickle speed depend on speed
uint32 CEMSocket::GetNeededBytes(bool lowspeed) {
	...
	uint64 timetotal = m_bAccelerateUpload?45000:90000;//Send 0.1kb trickle slot
	if (!lowspeed) 
		timetotal = m_bAccelerateUpload?10000:20000;//Send 0.5kb trickle slot
