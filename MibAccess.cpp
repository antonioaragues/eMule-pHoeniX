	...
	do
	{
		...
		if (!ret)
		{
			...
			if (!ret)
			{
				...
				MEMCOPY(m_pNICInfo[ifIndex].Description, varBind[4].value.asnValue.address.stream, i); 
...
void MibII::GetNICInfo(tSTRUCTNICINFO *pNICInfo)
{
	...
	for (i = 0; i < m_ifCount; i++)
	{
		MEMCOPY(tempStruct.IP, m_pNICInfo[i].IP, sizeof(tempStruct.IP)); 
		MEMCOPY(tempStruct.SubnetMask, m_pNICInfo[i].SubnetMask, sizeof(tempStruct.SubnetMask)); 
		...
		MEMCOPY(tempStruct.Description, m_pNICInfo[j].Description, sizeof(tempStruct.Description)); 
		tempStruct.MACLength = m_pNICInfo[j].MACLength;
		MEMCOPY(tempStruct.MAC, m_pNICInfo[j].MAC, tempStruct.MACLength); 
		...
		{
			MEMCOPY(&pNICInfo[validNICIndex], &tempStruct, sizeof(tSTRUCTNICINFO)); 
			validNICIndex++;
		}
	}
    ...
    )
{
  ...
  MEMCOPY(DstObjId->ids,SrcObjId->ids,SrcObjId->idLength*sizeof(UINT)); 
