...
CSearchFile::CSearchFile(const CSearchFile* copyfrom)
{
	...
	m_pszIsFake = copyfrom->GetFakeComment()? _tcsdup(copyfrom->GetFakeComment()) : NULL; // [TPT] - Fakecheck
	...
	m_detailSheetInterface = new CSearchResultFileDetailSheetInterface(this);	// [TPT] - SLUGFILLER: modelessDialogs
}
...
CSearchFile::CSearchFile(CFileDataIO* in_data, bool bOptUTF8, uint32 nSearchID, uint32 nServerIP, uint16 nServerPort, LPCTSTR pszDirectory, bool nKademlia)
{
	...
	m_pszIsFake = theApp.FakeCheck->IsFake(m_abyFileHash,GetFileSize()) ? _tcsdup(theApp.FakeCheck->GetLastHit()) : NULL;; // [TPT] - Fakecheck
	m_list_bExpanded = false;
	m_list_parent = NULL;
	m_list_childcount = 0;
	m_bPreviewPossible = false;
...
CSearchFile::CSearchFile(uint32 nSearchID, const uchar* pucFileHash, uint32 uFileSize, LPCTSTR pszFileName, int iFileType, int iAvailability)
{
	...
	m_pszDirectory = NULL;
	m_pszIsFake = theApp.FakeCheck->IsFake(m_abyFileHash,uFileSize) ? _tcsdup(theApp.FakeCheck->GetLastHit()) : NULL; // [TPT] - Fakecheck
	m_list_bExpanded = false;
	m_list_parent = NULL;
	m_list_childcount = 0;
	m_bPreviewPossible = false;
...
CSearchFile::~CSearchFile()
{
	...
	delete m_detailSheetInterface;	// [TPT] - SLUGFILLER: modelessDialogs
}
