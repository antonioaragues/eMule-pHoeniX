...
CBarShader::CBarShader(uint32 height, uint32 width): m_Spans(100) // [TPT] - Xman. Maella -Code Improvement (CPU load)- 
{
	m_iWidth = width;
	m_iHeight = height; // [TPT] - Patch	
	m_uFileSize = 0;	
	m_Spans.SetAt(0, 0);	// SLUGFILLER: speedBarShader
	m_Modifiers = NULL;
	m_bIsPreview=false;
	SetFileSize(1); // [TPT] - Patch	
}
