...
void CMMSocket::OnSend(int nErrorCode){
	if(m_pSendBuffer != NULL){
		...
		if (res == SOCKET_ERROR){
			...
			// [TPT]
			theApp.pBandWidthControl->AddeMuleOutTCPOverall(res);		
			m_nSent += res;
			if (m_nSent >= m_nSendLen){
				delete[] m_pSendBuffer;
				m_pSendBuffer = NULL;
...
void CListenMMSocket::DeleteClosedSockets(){
	...
	for (POSITION pos = m_socket_list.GetHeadPosition(); pos != 0;)
	{
		POSITION cur_pos = pos;
		CMMSocket* cur_sock = m_socket_list.GetNext(pos);	
	   if ( cur_sock->m_bClosed){
			m_socket_list.RemoveAt(cur_pos);
			delete cur_sock;
	   }
