...
: CResizableDialog(CStatisticsDlg::IDD, pParent), 
  ...
  m_UploadOMeter(FRIEND+1), // [TPT] - Friend data
  m_Statistics(4+1), // [TPT] - Memory consuming
  m_intervalGraph(0),
  m_intervalStat(0)
{
	sysinfo.Init(); // [TPT] - Memory consuming
}
// Maella end
...
void CStatisticsDlg::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("SEARCHMETHOD_SERVER")));					// Server
	iml.Add(CTempIconLoader(_T("FOLDERS")));			// Shared Files
...
BOOL CStatisticsDlg::OnInitDialog()
{
	...
	double emptyStatisticsPlot[4+1];
	for(int i=0; i<4+1; i++){
		emptyStatisticsPlot[i] = 0.0;
	}	
	m_Statistics.AppendEmptyPoints(emptyStatisticsPlot);	
	double emptyDLPlot[ADAPTER+1];
	for(int i=0; i<ADAPTER+1; i++){
		emptyDLPlot[i] = 0.0;
	}
	m_DownloadOMeter.AppendEmptyPoints(emptyDLPlot);	
	double emptyULPlot[FRIEND+1];
	for(int i=0; i<FRIEND+1; i++){
		emptyULPlot[i] = 0.0;
	}	
	m_UploadOMeter.AppendEmptyPoints(emptyULPlot);
	// Maella end
...
void CStatisticsDlg::DoResize_HL(int delta)
{
	...
	thePrefs.SetSplitterbarPositionStat_HL((int)floor((float)(rcspl.bottom*100.0f)/(float)rcW.Height())); // [TPT]
...
void CStatisticsDlg::DoResize_HR(int delta)
{
	...
	ScreenToClient(rcspl);
	thePrefs.SetSplitterbarPositionStat_HR((int)floor((float)(rcspl.bottom*100.0f)/(float)(rcW.Height()))); // [TPT]
...
void CStatisticsDlg::DoResize_V(int delta)
{
	...
	ScreenToClient(rcspl);
	thePrefs.SetSplitterbarPositionStat((int)floor((float)(rcspl.right*100.0f)/(float)rcW.Width())); // [TPT]
...
void CStatisticsDlg::RepaintMeters() 
{
	...
	m_UploadOMeter.SetPlotColor( thePrefs.GetStatsColor(13), FRIEND) ; // [TPT] - Friend data
	...
	m_Statistics.SetPlotColor( thePrefs.GetStatsColor(7), 4); // [TPT] - Memory consuming
	...
	m_UploadOMeter.SetLegendLabel(GetResString(IDS_ST_ULFRIEND),FRIEND); // [TPT] - Friend data
	...
	m_Statistics.SetLegendLabel(GetResString(IDS_MEMORY_CONSUMING),4); // [TPT] - Memory consuming
...
void CStatisticsDlg::ShowGraphs() {	
	...
	double plotStatistic[4+1]; // [TPT] - Memory consuming
	plotStatistic[0] = (double)theApp.listensocket->GetActiveConnections();
	plotStatistic[1] = (double)theApp.uploadqueue->GetActiveUploadsCount();
	plotStatistic[2] = (double)theApp.uploadqueue->GetUploadQueueLength();
	plotStatistic[3] = (double)myStats.a[1];
	
	// [TPT] - Memory consuming
	if(thePrefs.GetMemoryConsumingGraph() == true)
		plotStatistic[4] = (double)(sysinfo.GetProcessMemoryUsage()/1024); // in MB
	else
		plotStatistic[4] = 0;
	// Maella end
	
	// (pre)Update upload/download graphs
	double plotSentData[FRIEND+1]; // [TPT] - Friend data
	double plotReceivedData[ADAPTER+1]; 

	// Get Current datarates
	uint32 plotOutData[FRIEND+1]; // [TPT] - Friend data
	uint32 plotinData[ADAPTER+1];
	theApp.pBandWidthControl->GetDatarates(thePrefs.GetTrafficOMeterInterval(),
	...
	theApp.pBandWidthControl->GetFriendDataRate(thePrefs.GetTrafficOMeterInterval(), plotOutData[FRIEND]); // [TPT] - Friend data

	plotReceivedData[CURRENT] = (double)plotinData[CURRENT]/1024.0;
	plotSentData[CURRENT]     = (double)plotOutData[CURRENT]/1024.0;

	if(thePrefs.GetOverheadGraph() == true)
	{
	...
	plotSentData[FRIEND]	  = (double)plotOutData[FRIEND]/1024.0; // [TPT] - Friend data
...
void CStatisticsDlg::Process() {
	...
	if(::GetTickCount() > theApp.pBandWidthControl->GetStartTick() + 5000){
		...
		// we don´t need to show stats if we are not looking at statistic window
		if(theApp.emuledlg->activewnd == theApp.emuledlg->statisticswnd && theApp.emuledlg->IsWindowVisible())
		{
			if(thePrefs.GetStatsInterval() != 0 && thePrefs.GetStatsInterval() <= ++m_intervalStat)
			{
				m_intervalStat = 0;
				ShowStatistics();
			}
...
void CStatisticsDlg::ShowStatistics(bool forceUpdate)
{
	...
	// Update and save some statistic
	...
	if (forceUpdate || stattree.IsExpanded(h_transfer)) 
	{
		...
		// Transfer Ratios
		// [TPT] - WebCache // NOTE TO MODDERS: WEBCACHE DOWNLOADS CURRENTLY EFFECT TRANSFER RATIOS - Beware of UL:DL limits!!! relevant functions: thePrefs.GetDownData_WEBCACHE(); thePrefs.GetCumDownData_WEBCACHE();
		if ( theApp.pBandWidthControl->GeteMuleIn()>0 && theApp.pBandWidthControl->GeteMuleOut()>0 ) 
		{
			// Session
			if (theApp.pBandWidthControl->GeteMuleIn()<theApp.pBandWidthControl->GeteMuleOut()) 
			{
				cbuffer.Format(_T("%s %.2f : 1"),GetResString(IDS_STATS_SRATIO),(float)theApp.pBandWidthControl->GeteMuleOut()/theApp.pBandWidthControl->GeteMuleIn());
				stattree.SetItemText(trans[0], cbuffer);
			}
			else
			{
				cbuffer.Format(_T("%s 1 : %.2f"),GetResString(IDS_STATS_SRATIO),(float)theApp.pBandWidthControl->GeteMuleIn()/theApp.pBandWidthControl->GeteMuleOut());
				stattree.SetItemText(trans[0], cbuffer);
			}
		}
		else 
		...
		{
			// Session
			if (theApp.pBandWidthControl->GeteMuleOut() > theApp.pBandWidthControl->GeteMuleOutFriend() && theApp.pBandWidthControl->GeteMuleIn()<theApp.pBandWidthControl->GeteMuleOut()-theApp.pBandWidthControl->GeteMuleOutFriend()) 
			{
				cbuffer.Format(_T("%s %.2f : 1"),GetResString(IDS_STATS_FRATIO),(float)(theApp.pBandWidthControl->GeteMuleOut()-theApp.pBandWidthControl->GeteMuleOutFriend())/theApp.pBandWidthControl->GeteMuleIn());
				stattree.SetItemText(trans[1], cbuffer);
			}
			else
			{
				cbuffer.Format(_T("%s 1 : %.2f"),GetResString(IDS_STATS_FRATIO),(float)theApp.pBandWidthControl->GeteMuleIn()/(theApp.pBandWidthControl->GeteMuleOut()-theApp.pBandWidthControl->GeteMuleOutFriend()));
				stattree.SetItemText(trans[1], cbuffer);
			}
		}
		else
		...
		if ( (thePrefs.GetTotalDownloaded()>0 && thePrefs.GetTotalUploaded()>0) || (theApp.pBandWidthControl->GeteMuleIn()>0 && theApp.pBandWidthControl->GeteMuleOut()>0) ) 
		{
			// Cumulative
			if ((theApp.pBandWidthControl->GeteMuleIn()+thePrefs.GetTotalDownloaded())<(theApp.pBandWidthControl->GeteMuleOut()+thePrefs.GetTotalUploaded())) 
			{
				cbuffer.Format(_T("%s %.2f : 1"),GetResString(IDS_STATS_CRATIO),(float)(theApp.pBandWidthControl->GeteMuleOut()+thePrefs.GetTotalUploaded())/(theApp.pBandWidthControl->GeteMuleIn()+thePrefs.GetTotalDownloaded()));
				stattree.SetItemText(trans[2], cbuffer);
			}
		 	else
		 	{
				cbuffer.Format(_T("%s 1 : %.2f"),GetResString(IDS_STATS_CRATIO),(float)(theApp.pBandWidthControl->GeteMuleIn()+thePrefs.GetTotalDownloaded())/(theApp.pBandWidthControl->GeteMuleOut()+thePrefs.GetTotalUploaded()));
				stattree.SetItemText(trans[2], cbuffer);
			}
		}
		else
		...
		if (forceUpdate || stattree.IsExpanded(h_download)) 
		{
			...
			{
				// Downloaded Data
				cbuffer.Format( GetResString( IDS_STATS_DDATA ) , CastItoXBytes( theApp.pBandWidthControl->GeteMuleIn() ) );
				...
				if (forceUpdate || stattree.IsExpanded(down_S[4])) 
				{
					...
					if (statGoodSessions > 0) 
					{
						...
						cbuffer.Format(  _T("%s: %s") , GetResString(IDS_STATS_AVGDATADLSES) , CastItoXBytes( (uint64) theApp.pBandWidthControl->GeteMuleIn() / statGoodSessions, false, false ) ); 
					}
					else
					...
					if(thePrefs.GetVerbose() == true){
						cbuffer.AppendFormat(_T("(active: %u, paused: %u, no needed part: %u, corruption: %u, timeout: %u, cancelled: %u, out of part: %u, exception: %u, others: %u)"), 
									   myStats.a[1],
									   ...
									   CUpDownClient::GetDownStopCount(false, CUpDownClient::DSR_CANCELLED),
					...
					// Maella -Download Stop Reason-
					if(thePrefs.GetVerbose() == true){
						cbuffer.AppendFormat(_T("(paused: %u, no needed part: %u, corruption: %u, timeout: %u, cancelled: %u, out of part: %u, exception: %u, others: %u)"),
									   ...
									   CUpDownClient::GetDownStopCount(true, CUpDownClient::DSR_CANCELLED),
				...
				uint64 receivedSession = (uint64)theApp.pBandWidthControl->GeteMuleIn();
				cbuffer.Format(GetResString(IDS_STATS_GAINCOMP) + _T(" (%.1f%%)"), CastItoXBytes(thePrefs.GetSesSavedFromCompression(), false, false), receivedSession != 0 ? (thePrefs.GetSesSavedFromCompression() * 100.0 / receivedSession) : 0.0);
				stattree.SetItemText( down_S[5] , cbuffer );
				// Set Lost Due To Corruption
				cbuffer.Format(GetResString(IDS_STATS_LOSTCORRUPT) + _T(" (%.1f%%)"), CastItoXBytes(thePrefs.GetSesLostFromCorruption(), false, false), receivedSession != 0 ? (thePrefs.GetSesLostFromCorruption() * 100.0 / receivedSession) : 0.0);
			...
			{
				// Downloaded Data
				uint64 ullCumReceived = theApp.pBandWidthControl->GeteMuleIn()+thePrefs.GetTotalDownloaded();
		...
		if (forceUpdate || stattree.IsExpanded(h_upload)) 
		{
			...
			{
				// Uploaded Data
				cbuffer.Format(GetResString(IDS_STATS_UDATA),CastItoXBytes(theApp.pBandWidthControl->GeteMuleOut(), false, false));
				...
				cbuffer.Format(GetResString(IDS_STATS_UDATA_FRIENDS),CastItoXBytes(theApp.pBandWidthControl->GeteMuleOutFriend(), false, false));
				...
				if (forceUpdate || stattree.IsExpanded(up_S[5])) 
				{
					...
					{ // Blackholes are when God divided by 0
						percentSessions = (double) 100*statGoodSessions/(statGoodSessions+statBadSessions);
						cbuffer.Format(_T("%s: %s"), GetResString(IDS_STATS_AVGDATAULSES), CastItoXBytes((uint64) theApp.pBandWidthControl->GeteMuleOut() / statGoodSessions, false, false) ); 
					}
					else 
					{
						percentSessions = 0;
					...
					if(thePrefs.GetVerbose() == true){
						cbuffer.AppendFormat(_T("(active: %u, timeover: %u, new chunk: %u, cancelled/ended: %u, different file: %u, exception: %u, banned: %u, others: %u)"), 
									   theApp.uploadqueue->GetUploadQueueLength(),
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_TIMEOVER),
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_NEW_CHUNK),
									   ...
									   CUpDownClient::GetUpStopCount(false, CUpDownClient::USR_SNAFU), // [TPT] - added snafu
					...
					// Maella -Upload Stop Reason-
					if(thePrefs.GetVerbose() == true){
						cbuffer.AppendFormat(_T("(timeover: %u, new chunk: %u, cancelled/ended: %u, different file: %u, exception: %u, banned: %u, others: %u)"),
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_TIMEOVER),
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_NEW_CHUNK),
									   ...
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_SNAFU), // [TPT] - added snafu
									   statBadSessions - 
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_TIMEOVER) -
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_NEW_CHUNK) -
									   CUpDownClient::GetUpStopCount(true, CUpDownClient::USR_CANCELLED) -
			...
			{
				// Uploaded Data
				cbuffer.Format(GetResString(IDS_STATS_UDATA),CastItoXBytes( theApp.pBandWidthControl->GeteMuleOut()+thePrefs.GetTotalUploaded(), false, false));
				...
				if (forceUpdate || stattree.IsExpanded(up_T[1])) 
				{
					...
					{ // Blackholes are when God divided by 0
						percentSessions = (double) 100*statGoodSessions/(statGoodSessions+statBadSessions);
						cbuffer.Format(_T("%s: %s"), GetResString(IDS_STATS_AVGDATAULSES), CastItoXBytes((uint64) (theApp.pBandWidthControl->GeteMuleOut() + thePrefs.GetTotalUploaded()) / statGoodSessions, false, false) ); 
					}
					else 
					{
						percentSessions = 0;
	...
	{		
		...
		{			
			...
			if (forceUpdate || stattree.IsExpanded(hconn_su)) 
			{
				int i = 0;
				// Upload Rate
				cbuffer.Format(_T("%s: %s"), GetResString(IDS_ST_UPLOAD),CastItoXBytes(theStats.currentUploadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_su[i], cbuffer);
				i++;
				// Average Upload Rate
				cbuffer.Format(GetResString(IDS_STATS_AVGUL),CastItoXBytes(theStats.sessionUploadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_su[i], cbuffer); 
				i++;
				// Max Upload Rate
				cbuffer.Format(_T("%s: %s"), GetResString(IDS_STATS_MAXUL), CastItoXBytes(theStats.currentMaxUploadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_su[i], cbuffer);
				i++;
				// Max Average Upload Rate								
				cbuffer.Format(_T("%s: %s"), GetResString(IDS_STATS_MAXAVGUL), CastItoXBytes(theStats.sessionMaxUploadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_su[i], cbuffer); 
				i++;
			...
			if (forceUpdate || stattree.IsExpanded(hconn_sd)) 
			{
				int i = 0;
				// Download Rate
				cbuffer.Format(_T("%s: %s"), GetResString(IDS_ST_DOWNLOAD), CastItoXBytes(theStats.currentDownloadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_sd[i], cbuffer); 
				i++;
				// Average Download Rate
				cbuffer.Format(GetResString(IDS_STATS_AVGDL),CastItoXBytes(theStats.sessionDownloadRate, true, true));	 // [TPT]
				stattree.SetItemText(conn_sd[i], cbuffer); 
				i++;
				// Max Download Rate
				cbuffer.Format(GetResString(IDS_STATS_MAXDL),CastItoXBytes(theStats.currentMaxDownloadRate, true, true)); // [TPT]
				stattree.SetItemText(conn_sd[i], cbuffer); 
				i++;
				// Max Average Download Rate								
				cbuffer.Format(GetResString(IDS_STATS_MAXAVGDL), CastItoXBytes(theStats.sessionMaxDownloadRate, true, true));	// [TPT]
				stattree.SetItemText(conn_sd[i], cbuffer);
				i++;
		...
		if ( (forceUpdate || stattree.IsExpanded(htime_aap)) && timeDiff > 0 ) 
		{
			...
			for (int mx = 0; mx<3; mx++) 
			{
				if (forceUpdate || stattree.IsExpanded(time_aaph[mx])) 
				{
					...
					{
						// Uploaded Data
						cbuffer.Format(GetResString(IDS_STATS_UDATA),CastItoXBytes( (uint64)(theApp.pBandWidthControl->GeteMuleOut()+thePrefs.GetTotalUploaded())*avgModifier[mx], false, false));
	...
	if (forceUpdate || stattree.IsExpanded(h_clients)) 
	{
		...
		if (forceUpdate || stattree.IsExpanded(hclisoft)) 
		{
			...
			if (forceUpdate || stattree.IsExpanded(clisoft[0]) || cli_lastCount[0] == 0) 
			{
				...
				theApp.clientlist->ReleaseModStatistics(&clientMods);	// [TPT] - Slugfiller: modid
		...
		cbuffer.Format(_T("%s: %u"), GetResString(IDS_BANNED), theApp.clientlist->GetSnafuCount());
...
void CStatisticsDlg::SetARange(bool SetDownload, int maxValue)
{
	...
	else
	{
		...
		m_UploadOMeter.SetRange(0, maxValue+2, FRIEND); // [TPT] - Friend data
	}
}
// Maella end
...
void CStatisticsDlg::CreateMyTree() 
{
	...
	for(int i = 0; i<5/* changed 4 to 5 jp webcache statistics */; i++) 
		down_ssessions[i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), down_S[4]); // [TPT] - WebCache 
	...
	for(int x = 0; x<3; x++) 
	{
		...
		for(int i = 0; i<9/* changed 8 to 9 jp webcache statistics */; i++) 
			time_aap_down_dc[x][i] = stattree.InsertItem(GetResString(IDS_FSTAT_WAITING), time_aap_down_hd[x][0]); // [TPT] - WebCache 
