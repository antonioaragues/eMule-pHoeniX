...
BEGIN_MESSAGE_MAP(CED2kLinkDlg, CResizablePage) 
	...
	ON_BN_CLICKED(IDC_LD_HTMLCHE, OnHTMLChange) // [TPT] - Announ: -Copy BBCode ed2k links-
	ON_BN_CLICKED(IDC_LD_HOSTNAMECHE, OnSettingsChange)
	ON_BN_CLICKED(IDC_LD_HASHSETCHE, OnSettingsChange)
	ON_BN_CLICKED(IDC_LD_BBCODE, OnBBCODEChange) // [TPT] - Announ: -Copy BBCode ed2k links-
...
{ 
	...
	AddAnchor(IDC_LD_BBCODE, BOTTOM_LEFT, BOTTOM_LEFT); // [TPT] - Announ: -Copy BBCode ed2k links-
...
{ 
	...
	GetDlgItem(IDC_LD_BBCODE)->SetWindowText(GetResString(IDS_LD_BBCODE)); // [TPT] - Announ: -Copy BBCode ed2k links-
}
}
...
void CED2kLinkDlg::UpdateLink()
{
	...
	const bool bBBcode = ((CButton*)GetDlgItem(IDC_LD_BBCODE))->GetCheck() == BST_CHECKED; // [TPT] - Announ: -Copy BBCode ed2k links-
	...
	for (int i = 0; i != m_paFiles->GetSize(); i++){
		...
		const CKnownFile* file = STATIC_DOWNCAST(CKnownFile, (*m_paFiles)[i]);
		CString strCreateLink = CreateED2kLink(file, false);
		//[TPT] - Makes compatible with all forums ;)
		strCreateLink.Replace(_T("["), _T("%5B"));
		strCreateLink.Replace(_T("]"), _T("%5D"));

		strLinks += strCreateLink;
		...
		{
			strLinks += _T("]") + StripInvalidFilenameChars(file->GetFileName(), true) + _T("[/URL]");
		}
		// [TPT] - Announ: -Copy BBCode ed2k links-
	}
...
void CED2kLinkDlg::OnHTMLChange()
{
	if(IsDlgButtonChecked(IDC_LD_HTMLCHE))
		((CButton*)GetDlgItem(IDC_LD_BBCODE))->SetCheck(false);
	
	UpdateLink();

}

void CED2kLinkDlg::OnBBCODEChange()
{
	if(IsDlgButtonChecked(IDC_LD_BBCODE))
		((CButton*)GetDlgItem(IDC_LD_HTMLCHE))->SetCheck(false);
