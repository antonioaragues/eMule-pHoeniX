...
GeoIPRecord * _extract_record(GeoIP* gi, unsigned int seek_record, int *next_record_ptr) {
	...
	int dmaarea_combo = 0;
	size_t bytes_read = 0; // [TPT] - IP Country
	...
	record->latitude = (float)latitude/10000.0f - 180.0f; // [TPT] - IP Country
	record_buf += 3;

	/* get longitude */
	for (j = 0; j < 3; ++j)
		longitude += (record_buf[j] << (j * 8));
	record->longitude = (float)longitude/10000.0f - 180.0f; // [TPT] - IP Country
