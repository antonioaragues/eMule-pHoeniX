...
char *_full_path_to(const char *file_name) {
	...
	*q = 0;
	memset(path, 0, sizeof(char) * 1024);
	_snprintf(path, sizeof(char) * 1024 - 1, "%s/%s", buf, file_name);
#endif
...
void _setup_dbfilename() {
	if (NULL == GeoIPDBFileName) {
		...
		// [TPT] - IP Country
		GeoIPDBFileName[GEOIP_COUNTRY_EDITION] = "GeoIP.dat";
		GeoIPDBFileName[GEOIP_REGION_EDITION_REV0] = "GeoIPRegion.dat";
		GeoIPDBFileName[GEOIP_REGION_EDITION_REV1] = "GeoIPRegion.dat";
		GeoIPDBFileName[GEOIP_CITY_EDITION_REV0] = "GeoIPCity.dat";
		GeoIPDBFileName[GEOIP_CITY_EDITION_REV1] = "GeoIPCity.dat";
		GeoIPDBFileName[GEOIP_ISP_EDITION] = "GeoIPISP.dat";
		GeoIPDBFileName[GEOIP_ORG_EDITION] = "GeoIPOrg.dat";
		GeoIPDBFileName[GEOIP_PROXY_EDITION] = "GeoIPProxy.dat";
		GeoIPDBFileName[GEOIP_ASNUM_EDITION] = "GeoIPASNum.dat";
		// [TPT] - IP Country
	}
}
...
// [TPT] - IP Country
GeoIP* GeoIP_open_type (int type, int flags, const char* configDir) {
	GeoIP * gi = NULL;
	char * filePath = malloc(sizeof(char) * (strlen(configDir)+16));
	if (type < 0 || type >= NUM_DB_TYPES) {
		printf("Invalid database type %d\n", type);
		return NULL;
	}
	_setup_dbfilename();
	sprintf(filePath, "%s%s", configDir, GeoIPDBFileName[type]);
	if (filePath == NULL) {
		printf("Invalid database type %d\n", type);
		return NULL;
	}
...
GeoIP* GeoIP_new (int flags, const char* configDir) {
	GeoIP * gi;
	char * filePath = malloc(sizeof(char) * (strlen(configDir)+10));	
	_setup_dbfilename();
	sprintf(filePath, "%s%s", configDir, GeoIPDBFileName[GEOIP_COUNTRY_EDITION]); // [TPT] - IP Country
	gi = GeoIP_open (filePath, flags);
	return gi;
}
// [TPT] - IP Country
