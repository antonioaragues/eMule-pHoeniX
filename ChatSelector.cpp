...
BEGIN_MESSAGE_MAP(CChatSelector, CClosableTabCtrl)
	...
	ON_WM_CONTEXTMENU() // [TPT] - New Menu Styles 
END_MESSAGE_MAP()
...
{ 
	...
	bool friendSlot = ci->client && ci->client->GetFriendSlot();

	//Menu Configuration
	CMenuXP	*pChatMenu = new CMenuXP;
	pChatMenu->CreatePopupMenu();
	pChatMenu->SetMenuStyle(CMenuXP::STYLE_STARTMENU);
	pChatMenu->AddSideBar(new CMenuXPSideBar(17, _T("pHoeniX")));
	pChatMenu->SetSideBarStartColor(RGB(255,0,0));
	pChatMenu->SetSideBarEndColor(RGB(255,128,0));
	pChatMenu->SetSelectedBarColor(RGB(242,120,114));

		

	pChatMenu->AppendODMenu(MF_STRING | (isvalidconclient && !ci->client->IsFriend() )? MF_ENABLED : MF_GRAYED, new CMenuXPText(MP_FRIENDWINDOW_ADDFRIEND, GetResString(IDS_ADDFRIEND), theApp.LoadIcon(_T("friend2"), 16, 16)));
	pChatMenu->AppendODMenu(MF_STRING | isvalidconclient?MF_ENABLED : MF_GRAYED, new CMenuXPText(MP_FRIENDWINDOW_DETAILS, GetResString(IDS_SHOWDETAILS), theApp.LoadIcon(_T("details"), 16, 16)));
	if(friendSlot)
		pChatMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_FRIENDWINDOW_SLOTRM, GetResString(IDS_RM_FRIENDSLOT)));
	else
		pChatMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_FRIENDWINDOW_SLOT, GetResString(IDS_FRIENDSLOT)));

	pChatMenu->AppendSeparator();
	pChatMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_REMOVE, GetResString(IDS_FD_CLOSE), theApp.LoadIcon(_T("cruz"), 16, 16)));

	pChatMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, point.x, point.y, this);
...
BOOL CChatSelector::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	{ 
		...
		case MP_FRIENDWINDOW_DETAILS:
		{
			const CChatItem* ci = GetCurrentChatItem();
			if (ci)
				ci->client->GetDetailDialogInterface()->OpenDetailDialog(); //[TPT] - SLUGFILLER: ModelessDialogs
			return TRUE;			
		}
		case MP_FRIENDWINDOW_SLOT:
		{
			const CChatItem* ci = GetCurrentChatItem();		
			if (ci && ci->client->IsFriend())
			{				
				if( !ci->client->GetFriendSlot() )
				{
				theApp.friendlist->RemoveAllFriendSlots();
					ci->client->SetFriendSlot(true);
				}
			}
			return TRUE;
		}
		case MP_FRIENDWINDOW_SLOTRM:
		{
				theApp.friendlist->RemoveAllFriendSlots();
				return TRUE;
		}
		case MP_FRIENDWINDOW_ADDFRIEND:
		{
			const CChatItem* ci = GetCurrentChatItem();
			if (ci && !ci->client->IsFriend() )
