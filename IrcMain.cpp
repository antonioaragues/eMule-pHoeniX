		...
		if( command == _T("PRIVMSG") )
		{
			...
			if ( target.Left(1) == _T("#") )
			{
				...
				if( message.Left(1) == _T("\001") )
				{
					...
					{
						//Get client version.
						version = _T("eMule") + theApp.m_strCurVersionLong + _T(" ") + MOD_VERSION + _T(Irc_Version); // [TPT]- modID
						CString build;
			...
			else
			{
				...
				if( message.Left(1) == _T("\001") ){
					...
					if( message.Left(9) == _T("RQSFRIEND") )
					{
						...
						//Create our response.
						CString build;
						build.Format( _T("PRIVMSG %s :\001REPFRIEND eMule%s%s|%s|%u:%u|%s:%s|%s|\001"), source, theApp.m_strCurVersionLong + _T(" ") + MOD_VERSION, Irc_Version, sverify, theApp.IsFirewalled() ? 0 : theApp.GetID(), thePrefs.GetPort(), sip, sport, md4str(thePrefs.GetUserHash())); // [TPT]- modID
						ircsocket->SendString( build );
...
void CIrcMain::Connect()
{
	try
	{
		...
		nick = nick.Left(25);
		version = _T("eMule") + theApp.m_strCurVersionLong + _T(" ") + MOD_VERSION + (CString)Irc_Version; // [TPT]- modID
