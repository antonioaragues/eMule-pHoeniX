...
bool CWebCacheUpSocket::ProcessFirstHttpGet( const char* header, UINT uSize )
{
	...
	SetTimeOut(GetWebCacheSocketUploadTimeout());
	ClearHttpHeaders();
	SetHttpState( HttpStateRecvExpected );
...
////// Superlexx - encryption - end //////////////////////////////////////////////////////////////////	
	...
	//[TPT] - MORPH START - Changed by SiRoB, WebCache Fix
	/*
	...
	*/
	...
	//[TPT] - MORPH END - Changed by SiRoB, WebCache Fix
		DebugHttpHeaders(astrHeaders);
...
void CUpDownClient::OnWebCacheDownSocketTimeout()
{
	...
	//[TPT] - Sirob Fix on webcache
	if (GetDownloadState() == DS_DOWNLOADING && m_eWebCacheDownState == WCDS_DOWNLOADING	
 	    && !m_PendingBlocks_list.IsEmpty())
	{
		if (thePrefs.GetLogWebCacheEvents())
...
void CUpDownClient::PublishWebCachedBlock( const Requested_Block_Struct* block )
{
	...
	while( pos ) {
		...
		if( !cur_client->IsProxy()
			...
			if( cur_client->SupportsWebCacheUDP() && !cur_client->HasLowID() && thePrefs.GetUDPPort() != 0 )
			{
				data.WriteUInt32( cur_client->m_uWebCacheDownloadId );
				if (thePrefs.GetLogWebCacheEvents())
					AddDebugLogLine( false, _T("WCBlock sent to client - UDP %s"), reqfile->GetFileName()); // [TPT] - WebCache	
			...
			else
			{
				...
				if( cur_client->socket && socket->IsConnected() ) {
					if (thePrefs.GetLogWebCacheEvents())
						AddDebugLogLine( false, _T("WCBlock sent to client - TCP %s"), reqfile->GetFileName() ); // [TPT] - WebCache	
					cur_client->socket->SendPacket( packet );
				} else {
					if (thePrefs.GetLogWebCacheEvents())
						AddDebugLogLine( false, _T("WCBlock added to list - TCP %s"), reqfile->GetFileName() ); // [TPT] - WebCache	
