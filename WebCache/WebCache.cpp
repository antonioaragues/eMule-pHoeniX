	...
	if( wcip == 0) {
		...
		if( !remoteHost ) {
			if( !popupFlag) {
				...
				theApp.QueueLogLine(true, msg); // [TPT]
			}
	...
	if (hLib)
	{
		...
		if (pfnDnsQueryConfig && pfnDnsQuery && pfnDnsRecordListFree)
		{
			...
			if (nDnsState == 0)
			{
				...
				if (thePrefs.GetVerbose())
					theApp.QueueDebugLogLine(false, _T("WebCache ReverseDNS: Failed to resolve address \"%s\" - %s"), strDnsQuery, GetErrorMessage(nDnsState, 1));
			}
