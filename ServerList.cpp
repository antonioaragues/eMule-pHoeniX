...
#include "PartFile.h"	// [TPT] - itsonlyme: cacheUDPsearchResults
...
// [TPT] - serverOrder Sort servers based on SLUGFILLER sort
bool CServerList::CompareParts(POSITION pos1, POSITION pos2){
	bool ret = true;
	CServer* server1 = list.GetAt(pos1);
	CServer* server2 = list.GetAt(pos2);
	if ((server1->GetServerOrderNumber() == 0) && (server2->GetServerOrderNumber() != 0))
		ret = true;
	else if ((server1->GetServerOrderNumber() != 0) && (server2->GetServerOrderNumber() == 0))
		ret = false;
	else if (server1->GetServerOrderNumber() < server2->GetServerOrderNumber()) // lower order are the first
		ret = false;
	else
	{
		if ((server1->GetPreferences() > server2->GetPreferences()) && (server1->GetPreferences() != SRV_PR_LOW))
			ret = false;
		else
			ret = true;
	}
...
void CServerList::SwapParts(POSITION pos1, POSITION pos2){
	CServer* server1 = list.GetAt(pos1);
	CServer* server2 = list.GetAt(pos2);
	list.SetAt(pos1, server2);
	list.SetAt(pos2, server1);
}

void CServerList::HeapSort(uint16 first, uint16 last){
	uint16 r;
...
void CServerList::Sort(){
	...
	for ( i = n/2; i--; )
		HeapSort(i, n-1);
	for ( i = n; --i; ){
		SwapParts(list.FindIndex(0), list.FindIndex(i));
		HeapSort(0, i-1);
	}
}
// [TPT] - serverOrder
...
bool CServerList::SaveServermetToFile()
{
	...
	try{
		...
		for (UINT j = 0; j < fservercount; j++)
		{
			...
			servermet.WriteUInt16(nextserver->GetConnPort()); // [TPT] - Aux Ports
			...
			// [TPT] - serverOrder
			if (nextserver->GetServerOrderNumber()){
				CTag tagServerOrder(ST_SERVERORDER, nextserver->GetServerOrderNumber());
				tagServerOrder.WriteTagToFile(&servermet);
				uTagCount++;
			}
			// [TPT] - serverOrder
