...
BOOL CClientDetailPage::OnSetActive()
{
	...
	// [TPT] - IP Country
	GetDlgItem(IDC_DLOC)->SetWindowText(theApp.ip2country->GetCountryFromIP(client->GetCountryIP()));
	// [TPT] - IP Country
	...
	if (client->Credits()){
		...
		buffer.Format(_T("%.1f  [%.1f]"),(float)client->Credits()->GetScoreRatio(client->GetIP()),(float)client->Credits()->GetMyScoreRatio(client->GetIP()));	// [TPT] - VQB: ownCredits
	...
	if (client->GetUserName() && client->Credits()){
...
IMPLEMENT_DYNAMIC(CClientDetailDialog, CModelessPropertySheet)	// SLUGFILLER: modelessDialogs

BEGIN_MESSAGE_MAP(CClientDetailDialog, CModelessPropertySheet)	// SLUGFILLER: modelessDialogs
	ON_WM_DESTROY()
END_MESSAGE_MAP()

CClientDetailDialog::CClientDetailDialog(CUpDownClient* pClient, CListCtrlItemWalk* pListCtrl, UINT uInvokePage)	// itsonlyme:clientDetails
	: CModelessPropertySheet(pListCtrl)	// [TPT] - SLUGFILLER: modelessDialogs
{
	m_uInvokePage = uInvokePage;	// [tPT] - itsonlyme:clientDetails
	m_aItems.Add(pClient);
	Construct();
}

CClientDetailDialog::CClientDetailDialog(const CSimpleArray<CUpDownClient*>* paClients, CListCtrlItemWalk* pListCtrl, UINT uInvokePage)	// itsonlyme:clientDetails
	: CModelessPropertySheet(pListCtrl)	// [TPT] - SLUGFILLER: modelessDialogs
{
	m_uInvokePage = uInvokePage;	// [TPT] - itsonlyme:clientDetails
...
void CClientDetailDialogInterface::OpenDetailDialog(CListCtrlItemWalk* pListCtrl, UINT uInvokePage)	// [TPT] - itsonlyme:clientDetails
{
...
void CClientDetailDialogInterface::OpenDetailDialog(const CSimpleArray<CUpDownClient*>* paClients, CListCtrlItemWalk* pListCtrl, UINT uInvokePage)	// [TPT] - itsonlyme:clientDetails
{
	...
	OpenPropertySheet(&aInterfaces, paClients, pListCtrl, uInvokePage);	// itsonlyme:clientDetails
}
...
CModelessPropertySheet* CClientDetailDialogInterface::CreatePropertySheet(va_list args)
{
	...
	UINT uInvokePage = va_arg(args, UINT);	// [TPT] - itsonlyme:clientDetails
	return STATIC_DOWNCAST(CModelessPropertySheet, new CClientDetailDialog(paClients, pListCtrl, uInvokePage));	// [TPT] - itsonlyme:clientDetails
}
