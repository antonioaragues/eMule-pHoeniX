...
void CIrcWnd::DoResize(int delta)
{
	...
	if (!m_nicklist.IsColumnHidden(1)) // [TPT] - Fix
...
void CIrcWnd::AutoComplete()
{
	...
	// [TPT]
	for (POSITION pos = m_channelselect.m_pCurrentChannel->nicks.GetHeadPosition(); pos != NULL;)
	{
		POSITION cur_pos = pos;
		Nick* cur_nick = (Nick*)(m_channelselect.m_pCurrentChannel)->nicks.GetNext(pos);
...
void CIrcWnd::NoticeMessage( CString source, CString message )
{
	...
	// [TPT]
	for (POSITION pos = m_channelselect.channelPtrList.GetHeadPosition(); pos != NULL;)
	{
		POSITION cur_pos = pos;
		curr_channel = (Channel*)(m_channelselect.channelPtrList).GetNext(pos);
