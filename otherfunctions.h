...
__inline char* nstrdup(const char* todup){
   ...
   return (char*)MEMCOPY(new char[len], todup, len*sizeof(char));
}
...
// [TPT] - khaos::categorymod+ Takes a string and returns bytes...
...
__inline int isnulmd4(const void* hash) {
	return  (((uint64*)hash)[0] == 0 && ((uint64*)hash)[1] == 0);
}
...
enum EED2KFileType
{
	...
	ED2KFT_ARCHIVE,
	ED2KFT_CDIMAGE,
	ED2KFT_TORRENT // [TPT] - Bittorrent (waninkoko)
};

// [TPT] - MFCK [addon] - New Tooltips [Rayita] - Adapted by TPT
CString	YNStr(bool test);
HICON LoadSmallIcon(LPCTSTR lpszResourceName, bool shared = true);
HICON LoadLargeIcon(LPCTSTR lpszResourceName, bool shared = true);
...
bool stristrex (const TCHAR *s1, const TCHAR *s2); // [TPT] - eWombat SNAFU v2
...
long FileSize(LPCTSTR fileName); //[TPT] - Fakecheck
...
bool SetLayeredWindow(HWND hWnd,BYTE nAlpha=255);
// [TPT] - eWombat [MYINFOWND]

void Screenshot(CWnd * wnd, BOOL FullWnd); //[TPT] - eWombat [SCREENSHOT]
void	UpdateMSN(float upRate, float upOvRate, float downRate, float downOvRate, bool killMSN = false);//[TPT] - Show in MSN7
