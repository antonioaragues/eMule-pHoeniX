	...
	try
	{
		...
		if (_ParseURL(Data.sURL, _T("w")) == _T("password"))
		{
			...
			else if(thePrefs.GetWSIsLowUserEnabled() && thePrefs.GetWSLowPass()!=_T("") && MD5Sum(_ParseURL(Data.sURL, _T("p"))).GetHash() == thePrefs.GetWSLowPass())
			{
				...
				Login newban={inet_addr(T2CA(ip)), ::GetTickCount()};	// save failed attempt (ip,time)
			...
			if (login) {
				...
				{
					// Maella -Code Improvement-
					if (ipn == pThis->m_Params.badlogins[i].IP) {
		...
		else 
		{
			...
			for(int i = 0; i < pThis->m_Params.badlogins.GetSize();i++)
				if ( pThis->m_Params.badlogins[i].IP==ip ) faults++;
			// Maella end
			...
			if (faults>4) {
				...
				Login preventive={ip, ::GetTickCount() + (15*60*1000) }; // [TPT] - Maella -Code Improvement-
				for (int i=0;i<=5;i++)
...
CString CWebServer::_GetHeader(ThreadData Data, long lSession)
{
	...
	Out.Replace(_T("[version]"), theApp.m_strCurVersionLong + _T(" ") + MOD_VERSION); // [TPT]- modID
	...
	theApp.pBandWidthControl->GetDatarates(thePrefs.GetDatarateSamples(),
										   ...
										   eMuleOut, notUsed,
										   notUsed, notUsed);
    _stprintf(HTTPTempC, _GetPlainResString(IDS_UPDOWNSMALL), (float)eMuleOut/1024.0f, (float)eMuleIn/1024.0f);
	// Maella end
	...
	// Maella [FAF] -Allow Bandwidth Settings in <1KB Incremements-
		...
		{
			MaxUpload.Format(_T("%.1f"), thePrefs.GetMaxUpload());
		}
		else
		{
	...
	if(thePrefs.GetMaxDownload() < UNLIMITED){
		MaxDownload.Format(_T("%.1f"), thePrefs.GetMaxDownload());
	}
	else{
...
CString CWebServer::_GetTransferList(ThreadData Data,long lSession)
{
	...
	for (int i=0;i<partlist.GetCount();i++) {
		...
		if (cur_file) {
			...
			// [TPT]
			dFile.lFileSpeed = cur_file->GetDownloadFileDatarate(); // Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		...
		pos != 0;theApp.uploadqueue->GetNextFromUploadList(pos))
	{
		...
		// [TPT]
		fTotalSpeed += (float)cur_client->GetUploadDatarate(); // Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		HTTPTemp.Format(_T("%8.2f ") + _GetPlainResString(IDS_KBYTESEC), (float)cur_client->GetUploadDatarate()/1024.0f); // Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
		HTTPProcessData.Replace(_T("[4]"), HTTPTemp);
...
CString CWebServer::_GetPreferences(ThreadData Data)
{
	...
	if ((_ParseURL(Data.sURL, _T("saveprefs")) == _T("true")) && IsSessionAdmin(Data,sSession) ) {
		...
		{
			float maxdown = _tstof(_ParseURL(Data.sURL, _T("maxdown")));
			if(maxdown > 0.0f)
		...
		{
			float maxup = _tstof(_ParseURL(Data.sURL, _T("maxup")));
		...
		{
			thePrefs.SetMaxGraphDownloadRate(_tstof(_ParseURL(Data.sURL, _T("maxcapdown"))));
		}
		if(_ParseURL(Data.sURL, _T("maxcapup")) != _T(""))
		{
			thePrefs.SetMaxGraphUploadRate(_tstof(_ParseURL(Data.sURL, _T("maxcapup"))));
		}
		// Maella end
...
CString	CWebServer::_GetSearch(ThreadData Data)
{
	...
	Out.Replace(_T("[FileTypeTorId]"), _T(ED2KFTSTR_TORRENT));
	// [TPT] - Bittorrent (waninkoko)
