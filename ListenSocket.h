...
public:
	...
	void	Disconnect(LPCTSTR pszReason, CUpDownClient::UpStopReason reason = CUpDownClient::USR_NONE); // [TPT] - Maella - Stop Reason-
	void	WaitForOnConnect();
	// [TPT] - WebCache	
	// yonatan http -	void	ResetTimeOutTimer();
	...
	//CUpDownClient*	client; // [TPT]
protected:
	// [TPT]
	virtual void Close()	{CEMSocket::Close();} // deadlake PROXYSUPPORT - changed to AsyncSocketEx
	virtual	void OnInit();
	...
	void    PacketToDebugLogLine(bool isOpcodeKnown, const char* packet, uint32 size, UINT opcode, EDebugLogPriority dlpPriority);
	// Maella end
