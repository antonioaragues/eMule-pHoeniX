...
#include "Log.h"
#include "kademlia/kademlia/kademlia.h" // [TPT] - reconnect to Kademlia
...
void CServerConnect::ConnectToServer(CServer* server, bool multiconnect)
{
	...
	m_lstOpenSockets.AddTail(newsocket); // [TPT]
...
void CServerConnect::ConnectionEstablished(CServerSocket* sender)
{
	...
	if (sender->GetConnectionState() == CS_WAITFORLOGIN)
	{
		...
		CTag tagFlags(CT_SERVER_FLAGS,SRVCAP_ZLIB | SRVCAP_NEWTAGS | SRVCAP_AUXPORT);// [TPT] - Aux Ports (inform we are a "Aux Ports" compatible client
...
void CServerConnect::ConnectionFailed(CServerSocket* sender){
	...
	switch (sender->GetConnectionState()){
		case CS_FATALERROR:
			...
			Kademlia::CKademlia::stop(); // [TPT] - reconnect to Kademlia
			break;
...
{ 
	...
	try
	{
		if( thePrefs.GetNetworkKademlia() ) // [TPT] - reconnect to Kademlia
		{
...
{ 
	...
	while (pos){
		...
		// [TPT]
		// Maella -Code fix-
		if (tmpsock == NULL || tmpsock->cur_server == NULL) {
