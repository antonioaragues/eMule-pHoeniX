...
#include "sysinfo.h" // [TPT] - Memory consuming
...
public:
	...
	// <-----khaos-
	//[TPT] - moved here for instant change in vertical lines from preferences
	COScopeCtrl m_DownloadOMeter,m_UploadOMeter,m_Statistics;

private:
    // Maella -Accurate measure of bandwidth: eDonkey data + control, network adapter-
	enum Curve {CURRENT = 0, MINUTE = 1, SESSION = 2, OVERALL = 3, ADAPTER = 4, FRIEND = 5};
	...
	HTREEITEM	hdown_scb, down_scb[9]/*changed from 8 to 9 jp webcachestatistics */, hdown_spb, down_spb[2]; // Session Downloaded Byte Breakdowns // [TPT] - WebCache 
	HTREEITEM	hdown_tcb, down_tcb[9]/*changed from 8 to 9 jp webcachestatistics */, hdown_tpb, down_tpb[2]; // Total Downloaded Byte Breakdowns // [TPT] - WebCache 
	HTREEITEM	hdown_soh, down_soh[4], hdown_toh, down_toh[4]; // Downline Overhead
	HTREEITEM	down_ssessions[5]/*changed from 4 to 5 jp webcachestatistics */, down_tsessions[4], down_sources[22]; // Breakdown of Download Sessions and Sources // [TPT] - WebCache 
	...
	HTREEITEM	time_aap_down[3][7], time_aap_down_dc[3][9]/*changed from 8 to 9 jp webcachestatistics */, time_aap_down_dp[3][3]; // [TPT] - WebCache 
...
private:
	int m_lastRange[2];
	// Maella end
	CSysInfo sysinfo; // [TPT] - Memory consuming
};
