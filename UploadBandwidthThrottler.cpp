...
#include "emuledlg.h"
#include "BandWidthControl.h" // [TPT] - NAFC
...
// [TPT] - NAFC
void UploadBandwidthThrottler::SetNAFCEnable(bool newValue) {
	sendLocker.Lock();
...
void UploadBandwidthThrottler::SetNAFCdatarate(float newValue) {
	sendLocker.Lock();

	NAFCdatarate = newValue;
...
void UploadBandwidthThrottler::SetCumulateBandwidth(bool newValue) {
	sendLocker.Lock();
...
// [TPT] -MTU Configuration-
void UploadBandwidthThrottler::SetMTUValue(uint32 newValue) {
	sendLocker.Lock();
	...
	if(doRun) {
        // Remove this socket from control packet queue
        {
	        ...
	        for (POSITION pos = m_ControlQueue_list.GetHeadPosition(); pos != NULL;) 
			{
		        ...
		        ThrottledControlSocket* socketinQueue = m_ControlQueue_list.GetNext(pos);
                if(socketinQueue == socket)
                    m_ControlQueue_list.RemoveAt(cur_pos);                
            }
        }
        ...
        {            
	        for (POSITION pos = m_ControlQueueFirst_list.GetHeadPosition(); pos != NULL;) {
		        POSITION cur_pos = pos;
		        ThrottledControlSocket* socketinQueue = m_ControlQueueFirst_list.GetNext(pos);

                if(socketinQueue == socket) {
                    m_ControlQueueFirst_list.RemoveAt(cur_pos);
                }
            }
        }
        ...
        {            
	        for (POSITION pos = m_TempControlQueue_list.GetHeadPosition(); pos != NULL;) {
		        POSITION cur_pos = pos;
		        ThrottledControlSocket* socketinQueue = m_TempControlQueue_list.GetNext(pos);

                if(socketinQueue == socket) {
                    m_TempControlQueue_list.RemoveAt(cur_pos);
                }
            }
        }
        ...
        {            
	        for (POSITION pos = m_TempControlQueueFirst_list.GetHeadPosition(); pos != NULL;) {
		        POSITION cur_pos = pos;
		        ThrottledControlSocket* socketinQueue = m_TempControlQueueFirst_list.GetNext(pos);

                if(socketinQueue == socket) {
                    m_TempControlQueueFirst_list.RemoveAt(cur_pos);
                }
            }
        }
		tempQueueLocker.Unlock();
...
UINT UploadBandwidthThrottler::RunInternal() {
	...
	cumulateBW = thePrefs.IsCumulateBandwidthEnabled();  // [TPT] - Cumulate Bandwidth
	minFragMTU = thePrefs.GetMTU(); // [TPT] -MTU Configuration-
	nafcEnable = thePrefs.GetNAFCEnable();
	...
	while(doRun) {
		...
		if(nafcEnable)
			allowedDataRate = NAFCdatarate;
		else
			allowedDataRate = theApp.lastCommonRouteFinder->GetUpload();

		// Maella -MTU Configuration-
		// Remark: code simplification => don't care of UDP
		uint32 minFragSize = minFragMTU - (20 /*IP*/ + 20 /*TCP*/);
		//[TPT] - Only send one packet, better upload
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
        ...
        if(bytesToSpend >= 1) {
		    ...
		    for(uint32 slotCounter = 0; slotCounter < (uint32)m_StandardOrder_list.GetSize(); slotCounter++) {
			    ...
			    if(socket != NULL) {
				    if(thisLoopTick-socket->GetLastCalledSend() > SEC2MS(1)) {
					    // trickle
					    uint32 neededBytes = socket->GetNeededBytes(minFragSize<1000);//From Morph
			...
			// [TPT] - Cumulate Bandwidth
			if (cumulateBW && (m_StandardOrder_list.GetSize() > 0) && 
				(bytesToSpend > 0) && (spentBytes < (uint64)bytesToSpend))
				{					
				ThrottledFileSocket* socket = m_StandardOrder_list.GetAt(0); // Feed the BW to first slot
					...
					theApp.QueueDebugLogLine(false,_T("There was a NULL socket in the UploadBandwidthThrottler Standard list (equal-for-all)! Prevented usage. Index: %i Size: %i"), 0, m_StandardOrder_list.GetSize());
					continue;
					}
				}
			// [TPT] - Cumulate Bandwidth
