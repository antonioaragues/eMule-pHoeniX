...
void CConnectionWizardDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_WIZ_TRUEDOWNLOAD_BOX, m_trueDownload);
	DDX_Control(pDX, IDC_WIZ_TRUEUPLOAD_BOX, m_trueUpload);
	// Maella end
}
...
void CConnectionWizardDlg::OnBnClickedApply()
{
	...
	// Retrieve Download
	CString buffer;
	if(GetDlgItem(IDC_WIZ_TRUEDOWNLOAD_BOX)->GetWindowTextLength() > 0){ 
	...
	// Retrieve Upload
	if(GetDlgItem(IDC_WIZ_TRUEUPLOAD_BOX)->GetWindowTextLength() > 0){ 
	...
	if( upload > 0.0f && download > 0.0f ){
		thePrefs.SetMaxGraphUploadRate(upload);
		thePrefs.SetMaxGraphDownloadRate(download);

		thePrefs.SetMaxUpload(0.8f * upload);
		thePrefs.SetMaxDownload(download); 
		thePrefs.SetMaxDownload(thePrefs.GetMaxDownload()); // Apply the ratio anti-lamer
		...
		else if( upload < 12.0f )
			thePrefs.maxconnections = 200;	
		else if( upload < 25.0f )
			thePrefs.maxconnections = 400;
		else if( upload < 37.0f )
...
BOOL CConnectionWizardDlg::OnInitDialog()
{
	...
	// Maella [FAF] -Allow Bandwidth Settings in <1KB Incremements-
	CString temp;
	temp.Format(_T("%.1f"), (int)(8.0f * thePrefs.GetMaxGraphUploadRate())); // kBits/s
