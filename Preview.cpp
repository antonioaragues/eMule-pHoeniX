...
int CPreviewApps::GetAllMenuEntries(CMenuXP* rMenu, const CPartFile* file) // [TPT] - New Menu Styles
{
	...
	for (int i = 0; i < m_aApps.GetCount(); i++)
	{
		...
		rMenu->AppendODMenu(MF_STRING | (bEnabled ? MF_ENABLED : MF_GRAYED), new CMenuXPText(MP_PREVIEW_APP_MIN + i, rSvc.strTitle)); // [TPT] - New Menu Styles
	}
	return m_aApps.GetCount();
}
