...
int CWebServices::GetAllMenuEntries(CMenuXP* rMenu, DWORD dwFlags)
{
	...
	for (int i = 0; i < m_aServices.GetCount(); i++)
	{
		...
		if ((dwFlags & WEBSVC_FILE_URLS) && !rSvc.bFileMacros)
			continue;
		if (rMenu->AppendODMenu(MF_STRING, new CMenuXPText(MP_WEBURL + i,rSvc.strMenuLabel)))
			iMenuEntries++;
	}
	return iMenuEntries;
}
...
} _aED2KFileTypes[] = 
{
	...
	// [TPT] - Bittorrent (waninkoko)
	{ _T(".torrent"), ED2KFT_TORRENT },
	{ _T(".tor"),     ED2KFT_TORRENT }
	// [TPT] - Bittorrent (waninkoko)
};
...
LPCSTR GetED2KFileTypeSearchTerm(EED2KFileType iFileID)
{
	...
	// [TPT] - Bittorrent (waninkoko)
	if (iFileID == ED2KFT_TORRENT)          return ED2KFTSTR_PROGRAM;
	// [TPT] - Bittorrent (waninkoko)
	return NULL;
}
...
CString GetFileTypeByName(LPCTSTR pszFileName)
{
	...
	switch (iFileType) {
		...
		// [TPT] - Bittorrent (waninkoko)
		case ED2KFT_TORRENT:    return _T(ED2KFTSTR_TORRENT);
		// [TPT] - Bittorrent (waninkoko)
		default:				return _T("");
	}
}
...
CString	YNStr(bool test)
{
	return GetResString(test ? IDS_YES : IDS_NO);
}

HICON LoadSmallIcon(LPCTSTR lpszResourceName, bool shared)
{
	return theApp.LoadIcon(lpszResourceName, 16, 16, shared ? LR_DEFAULTCOLOR : LR_DEFAULTCOLOR | LR_SHARED);
}

HICON LoadLargeIcon(LPCTSTR lpszResourceName, bool shared)
{
	return theApp.LoadIcon(lpszResourceName, 32, 32, shared ? LR_DEFAULTCOLOR : LR_DEFAULTCOLOR | LR_SHARED);
}
...
// [TPT]
// Maella -Fake Search- (sivka)
void FakeCheck1(CAbstractFile* file)
{
	TCHAR url[150], hash[33]=_T("");
	md4str(file->GetFileHash(), hash);	
	_stprintf(url, _T("http://donkeyfakes.gambri.net/index.php?action=search&hash=%s"), hash);				
	ShellOpenFile(url);
}
...
void RGB2HSL(COLORREF color, double *h, double *s, double *l) {
	double r, g, b;
...
COLORREF HSL2RGB(double h, double s, double l) {
	...
	if (s == 0)
		...
		if (6.0f * tmp3 < 1.0f)
			r = (tmp1+(tmp2-tmp1)*tmp3*6.0f);
		else if (2.0f * tmp3 < 1.0f)
			r = tmp2;
		else if (3.0f * tmp3 < 2.0f)
...
COLORREF GetHighContrastColor(COLORREF color1, COLORREF color2){
	double h1=0, s1=0, l1=0;
	double h2=0, s2=0, l2=0;
	double pi = 3.1415926535;

	RGB2HSL(color1, &h1, &s1, &l1);
	RGB2HSL(color2, &h2, &s2, &l2);

	if((h1!=0.0f || s1!=0.0f) && (h2!=0.0f || s2!=0.0f)){
		if(s1>s2){
			if((s1-s2) < 0.5){
				if(s1>0.5)
					s2=s1-0.5-(s1-s2);
	...
	return HSL2RGB(h2,s2,l2);
}
// [TPT] - MoNKi: -Support for High Contrast Mode-

// [TPT] - eWombat [MYINFOWND]
typedef BOOL (WINAPI *lpfnSetLayeredWindowAttributes)(HWND hWnd, COLORREF crKey, BYTE bAlpha, DWORD dwFlags);
static lpfnSetLayeredWindowAttributes m_pSetLayeredWindowAttributes;
static BOOL m_bWin2K;
//Enable Layered Window Mode without PLATFORM-SDK
//Any OS prior to Win2K will ignore it 
bool SetLayeredWindow(HWND hWnd,BYTE nAlpha)
{
	...
	if (!m_bWin2K || (NULL == m_pSetLayeredWindowAttributes))
		return FALSE; //Bail out!!!
	if (hWnd==NULL)
		return FALSE;

	SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
	// Sets the window to nAlpha (0=TRANSPARENT..255 OPAQUEUE) visibility.
	m_pSetLayeredWindowAttributes(hWnd,0,nAlpha, LWA_ALPHA);
	return TRUE;
}
// [TPT] - eWombat [MYINFOWND]
...
void Screenshot(CWnd * wnd, BOOL FullWnd)
{
     CDC dc;
     if(FullWnd)
        { /* full window */
...
// [TPT] end eWombat [SCREENSHOT]


//[TPT] - Show in MSN7 (ikabot/MoNKi)
...
void UpdateMSN(float upRate, float upOvRate, float downRate, float downOvRate, bool killMSN)
{
	...
	if(thePrefs.ShowOverhead())
	{
		...
		MSNStr.Format(MSNTemplateString, bshow, MSNFormat, MOD_VERSION, upRate, upOvRate, downRate, downOvRate);
	}
	else
	{
		MSNTemplateString += GetResString(IDS_UPDOWNSMALL) + L"\\0";
		MSNStr.Format(MSNTemplateString, bshow, MSNFormat, MOD_VERSION, upRate, downRate);
	}
