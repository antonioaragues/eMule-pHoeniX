...
{ 
	...
	// no 'Close' for status log and channel list
	pMenu->AppendODMenu(MF_STRING | (iCurTab < 2) ? MF_GRAYED : MF_ENABLED, new CMenuXPText(Irc_Close, GetResString(IDS_FD_CLOSE), theApp.LoadIcon(_T("delete"), 16, 16)));
	...
	if( totallength > index )
	{
		...
		for( int i = 0; i < length; i++)
		{
			...
			{
				pMenu->AppendODMenu(MF_STRING, new CMenuXPText(Irc_ChanCommands+index, _T("Set +") + mode + _T(" ( Not Supported Yet )")));
				pMenu->EnableMenuItem(Irc_ChanCommands+index, MF_GRAYED);
			}
			else
			{
				pMenu->AppendODMenu(MF_STRING, new CMenuXPText(Irc_ChanCommands+index+50, _T("Set -") + mode + _T(" ( Not Supported Yet )")));
				pMenu->EnableMenuItem(Irc_ChanCommands+index+50, MF_GRAYED);
			}
			index++;
		}
		...
		for( int i = 0; i < length; i++)
		{
			...
			{
				pMenu->AppendODMenu(MF_STRING, new CMenuXPText(Irc_ChanCommands+index+50, _T("Set -") + mode + _T(" ( Remove ") + mode + _T(" )")));
			}
			index++;
		}
		...
		for( int i = 0; i < length; i++)
		{
			...
			{
				pMenu->AppendODMenu(MF_STRING, new CMenuXPText(Irc_ChanCommands+index, _T("Set +") + mode + _T(" ( Add ") + mode + _T(" )")));
			}
			else
			{
				pMenu->AppendODMenu(MF_STRING, new CMenuXPText(Irc_ChanCommands+index+50, _T("Set -") + mode + _T(" ( Remove ") + mode + _T(" )")));
...
void CIrcChannelTabCtrl::OnMeasureItem(int nIDCtl, LPMEASUREITEMSTRUCT lpMeasureItemStruct) 
{
...
void CIrcChannelTabCtrl::DeleteAllChannel()
{
	...
	for (POSITION pos = channelPtrList.GetHeadPosition(); pos != NULL;)
	{
		...
		channelPtrList.RemoveAt(cur_pos);
		delete cur_channel;
	}
}
